...
public class JMXHeritrixController extends AbstractJMXHeritrixController {
    ...
    private void initializeProgressStatisticsLegend() {
        new Thread() {
            public void run() {
                progressStatisticsLegend = (String) executeCrawlJobCommand(PROGRESS_STATISTICS_LEGEND_COMMAND);
            }
        }.start();
    }
    ...
    public boolean isPaused() {
        ...
        return status != null && (status.equals(PAUSED_STATUS) || status.equals(PAUSING_STATUS));
    }
    ...
    private String getJobName() {
        ...
        // jobs list.
        if (pendingJobs == null || pendingJobs.size() == 0) {
            throw new IOFailure("Heritrix has not created a job after " + (Math.pow(2, JMXUtils.getMaxTries()) / 1000)
            		+ " seconds, giving up.");
    ...
    private String getJMXAdminName() {
        ...
        log.debug("The JMX username used for connecting to the Heritrix GUI is: '{}'.", jmxUsername);
        return jmxUsername;
        }
    ...
    private Object executeHeritrixCommand(String command, String... arguments) {
        return JMXUtils.executeCommand(getHeritrixJMXConnector(), getHeritrixBeanName(), command, arguments);
    }
    ...
    private String getCrawlJobBeanName() {
        return "org.archive.crawler:" + JmxUtils.NAME + "=" + jobName + "," + JmxUtils.TYPE + "=CrawlService.Job,"
        		+ JmxUtils.JMX_PORT + "=" + getJMXPort() + "," + JmxUtils.MOTHER + "=Heritrix,"
        		+ JmxUtils.HOST + "=" + getHostName();
    }
    ...
    private JMXConnector getHeritrixJMXConnector() {
        return JMXUtils.getJMXConnector(SystemUtils.LOCALHOST, getJMXPort(), getJMXAdminName(), getJMXAdminPassword());
    }
