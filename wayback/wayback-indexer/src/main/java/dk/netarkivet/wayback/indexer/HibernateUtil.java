        ...
        if (sessionFactory == null || sessionFactory.isClosed()) {
            try {
                ...
                log.info("Hibernate properties used: " + CONNECTION_PROVIDER_CLASS + " = "
                        ...
                        + Settings.get(WaybackSettings.C3P0_ACQUIRE_INCREMENT) + ", " + C3P0_IDLE_TEST_PERIOD + " = "
                        + Settings.get(WaybackSettings.C3P0_IDLE_PERIOD) + ", " + C3P0_MAX_SIZE + " = "
                        + Settings.get(WaybackSettings.C3P0_MAX_SIZE) + ", " + C3P0_MAX_STATEMENTS + " = "
                        + Settings.get(WaybackSettings.C3P0_MAX_STATEMENTS) + ", " + C3P0_MIN_SIZE + " = "
                        + Settings.get(WaybackSettings.C3P0_MIN_SIZE) + ", " + C3P0_TIMEOUT + " = "
                        + Settings.get(WaybackSettings.C3P0_TIMEOUT) + ", " + HIBERNATE_CONNECTION_DRIVER_CLASS + " = "
                        + Settings.get(WaybackSettings.HIBERNATE_DB_DRIVER) + ", " + HIBERNATE_CONNECTION_URL + " = "
                        + Settings.get(WaybackSettings.HIBERNATE_DB_URL) + ", " + HIBERNATE_DIALECT + " = "
                        + Settings.get(WaybackSettings.HIBERNATE_DIALECT) + ", " + HIBERNATE_FORMAT_SQL + " = "
                        + Settings.get(WaybackSettings.HIBERNATE_FORMAT_SQL) + ", "
                        + HIBERNATE_BYTECODE_USE_REFLECTION_OPTIMIZER + " = "
                        + Settings.get(WaybackSettings.HIBERNATE_REFLECTION_OPTIMIZER) + ", " + HIBERNATE_HBM2DDL_AUTO
                        + " = " + Settings.get(WaybackSettings.HIBERNATE_HBM2DDL_AUTO) + ", "
                        + HIBERNATE_TRANSACTION_FACTORY_CLASS + " = "
                        + Settings.get(WaybackSettings.HIBERNATE_TRANSACTION_FACTORY) + ", " + HIBERNATE_SHOW_SQL
