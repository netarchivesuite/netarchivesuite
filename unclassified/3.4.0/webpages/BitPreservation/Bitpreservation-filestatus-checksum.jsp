...
%><%
    ...
    try {
        HTMLUtils.forwardOnIllegalParameter(pageContext,
    ...
    BitpreserveFileStatus.processChecksumRequest(request, res, pageContext);
...
<%
    ...
    if (filename != null) {
        ...
        FilePreservationStatus fs = preserve.getFilePreservationStatus(
                filename);
        if (fs == null) {
%><fmt:message key="no.info.on.file.{0}">
            ...
            <br><br>
            <table>
            <tr><td>&nbsp;</td><td><fmt:message key="admin.state"/></td><td><fmt:message key="checksum"/></td></tr>
            <!-- General admin state not used because is is never updated -->
            <tr><td>&nbsp;</td><td>&nbsp;</td>
            <%=HTMLUtils.makeTableElement(fs.getAdminChecksum())%>
            </tr>
            <%
            for (Location l : Location.getKnown()) {
                String csumString = HTMLUtils.presentChecksum(
...
<h4><fmt:message key="files.with.checksum.errors.in.0"><fmt:param><%=bitarchive%></fmt:param></fmt:message></h4>
<%
    List<String> wrongFiles = BitpreserveFileStatus.getWrongFilesList(
            bitarchive, pageContext);
    for (String fn : wrongFiles) { %>
    &nbsp;<%=fn%>&nbsp;<a href="./Bitpreservation-filestatus-checksum.jsp?<%=
    Constants.BITARCHIVE_NAME_PARAM%>=<%=HTMLUtils.encodeAndEscapeHTML(bitarchive.getName())%>&amp;<%=
    Constants.FILENAME_PARAM%>=<%=HTMLUtils.encodeAndEscapeHTML(fn)%>"><fmt:message key="info"/></a><br/>
	<%  } // end for
