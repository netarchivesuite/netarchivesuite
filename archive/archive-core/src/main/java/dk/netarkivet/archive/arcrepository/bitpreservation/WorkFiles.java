...
public enum WorkFiles {
    ...
    protected static File getPreservationDir(Replica replica) {
        File base = new File(Settings.get(ArchiveSettings.DIR_ARCREPOSITORY_BITPRESERVATION));
    ...
    protected static File getSortedFile(File unsortedFile) throws IOFailure {
        ...
        File sortedOutput = new File(unsortedFile.getParentFile(), SORTED_OUTPUT_FILE);
        if (!unsortedFile.exists()) {
            throw new IOFailure("Unsorted input file '" + unsortedFile + "' does not exist");
        }
    ...
    public static void write(Replica replica, WorkFiles fileType, Set<String> files) {
        ...
        FileUtils.writeCollectionToFile(getFile(replica, fileType), files);
    }
