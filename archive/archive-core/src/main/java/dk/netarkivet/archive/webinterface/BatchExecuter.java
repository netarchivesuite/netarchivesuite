...
public class BatchExecuter extends Thread {
    ...
    public void run() throws IOFailure {
        ...
        if (batchJob instanceof LoadableJarBatchJob) {
            ...
            log.debug("LoadableJarBatchJob is actually the batchjob '" + jobName + "'.");
        }

        try {
            // create output and error files.
            File outputFile = new File(BatchGUI.getBatchDir(), jobName + Constants.NAME_TIMSTAMP_SEPARATOR + timestamp
                    + Constants.OUTPUT_FILE_EXTENSION);
            outputFile.createNewFile();
            File eventLogFile = new File(BatchGUI.getBatchDir(), jobName + Constants.NAME_TIMSTAMP_SEPARATOR
                    + timestamp + Constants.ERROR_FILE_EXTENSION);
            ...
            String processInfo = "Starting batchjob '" + jobName + "' at time '" + timestamp + "' on replica '"
            ...
            // log results.
            processInfo = "Successfully finished BatchJob '" + jobName + "' on " + status.getNoOfFilesProcessed()
                    + " files.";
            log.info(processInfo);
            fw.write(processInfo + "\n");
            // log status.
            processInfo = "BatchJob '" + jobName + "' has failed on '" + failedFiles.size()
                    + "' files and has gotten '" + exceptions.size() + "' exceptions.";
            log.info(processInfo);
            fw.write(processInfo + "\n");
            ...
            if (outputFile != null && outputFile.exists()) {
                ...
                log.warn("Could not print output to file. Logging it instead: " + "'\n:"
                        + StreamUtils.getInputStreamAsString(status.getResultFile().getInputStream()));
            }
            ...
            // print exceptions
            log.info("BatchJob '" + jobName + "' encountered " + exceptions.size() + " exceptions.");
            ...
            String errMsg = "Could not handle batchjob '" + jobName + "' with timestamp '" + timestamp + "'.";
            log.error(errMsg, e);
