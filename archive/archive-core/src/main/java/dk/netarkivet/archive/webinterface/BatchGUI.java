...
public final class BatchGUI {
    ...
    public static void getPageForClass(PageContext context) throws UnknownID, ArgumentNotValid, IllegalState,
            ForwardedToErrorPage, IOFailure {
        ArgumentNotValid.checkNotNull(context, "PageContext context");
    ...
    private static Class getBatchClass(String className) throws UnknownID, IllegalState {
        ...
        if (!(FileBatchJob.class.isAssignableFrom(res))) {
            String errMsg = "The class '" + className + "' is not an instance " + "of '" + FileBatchJob.class.getName()
                    + "' as required.";
            log.warn(errMsg);
    ...
    private static String getClassDescription(Class c, Locale locale) {
        ...
        for (Resource resource : r.value()) {
            if (resource.type().getName().equals(c.getName())) {
                return I18N.getString(locale, "batchpage;Description", new Object[] {}) + ": " + resource.description()
                        + "<br/><br/>\n";
            }
        }
        ...
        if (resource.length < params.length) {
            ...
            if (parmIndex < params.length) {
                for (int i = parmIndex + 1; i <= params.length; i++) {
                    res.append(I18N.getString(locale, "batchpage;Argument.i.missing.argument.metadata", i) + "<br/>\n");
    ...
    private static String getPreviousRuns(String jobPath, Locale locale) {
        ...
        for (String prefix : prefixes) {
            ...
            if (!outputFile.exists()) {
                res.append("    <td>" + I18N.getString(locale, "batchpage;No.outputfile", new Object[] {}) + "</td>\n");
                res.append("    <td>" + I18N.getString(locale, "batchpage;No.outputfile", new Object[] {}) + "</td>\n");
                res.append("    <td>" + I18N.getString(locale, "batchpage;No.outputfile", new Object[] {}) + "</td>\n");
            } else {
            ...
            if (!errorFile.exists()) {
                res.append("    <td>" + I18N.getString(locale, "batchpage;No.errorfile", new Object[] {}) + "</td>\n");
                res.append("    <td>" + I18N.getString(locale, "batchpage;No.errorfile", new Object[] {}) + "</td>\n");
                res.append("    <td>" + I18N.getString(locale, "batchpage;No.errorfile", new Object[] {}) + "</td>\n");
            } else {
    ...
    private static String getTimestamp(String prefix, Locale locale) {
        ...
        if (split.length >= 2) {
            try {
                ...
                log.warn("Could not parse batchjob result file name: " + prefix, e);
            }
        }

        // If no timestamp was identified, write an error-message instead
        if (timestamp.isEmpty()) {
            timestamp = I18N.getString(locale, "batchpage;No.valid.timestamp", new Object[] {});
        }
        return timestamp;
    }
    ...
    private static String getReplicaRadioButtons(Locale locale) {
        ...
        res.append(I18N.getString(locale, "batchpage;Choose.replica", new Object[] {}) + ": <br/>\n");

        // Make radio buttons and categorize them as replica.
        for (Replica rep : Replica.getKnown()) {
            res.append("<input type=\"radio\" name=\"replica\" value=\"" + rep.getName() + "\"");
            // Disable for checksum replica.
            if (rep.getType().equals(ReplicaType.CHECKSUM)) {
                res.append(" disabled");
            } else if (rep.getId().equals(Settings.get(CommonSettings.USE_REPLICA_ID))) {
                res.append(" checked");
            }
    ...
    private static String getRegularExpressionInputBox(Locale locale) {
        ...
        res.append(I18N.getString(locale, "batchpage;Which.files", new Object[] {}));
        res.append(":<br/>\n");

        // Make job id input:
        res.append(I18N.getString(locale, "batchpage;Job.ID", new Object[] {}) + " &nbsp; &nbsp; &nbsp; <input name=\""
                + Constants.JOB_ID_PARAMETER + "\" size=\"25\" value=\"1\" " + "/><br/>\n");

        // Add metadata option (checked radiobutton)
        res.append("<input type=\"radio\" name=\"filetype\" value=\"" + BatchFileType.Metadata + "\" checked />"
                + I18N.getString(locale, "batchpage;Metadata", new Object[] {}) + "<br/>\n");
        // Add content option
        res.append("<input type=\"radio\" name=\"filetype\" value=\"" + BatchFileType.Content + "\" />"
                + I18N.getString(locale, "batchpage;Content", new Object[] {}) + "<br/>\n");
        // Add both option
        res.append("<input type=\"radio\" name=\"filetype\" value=\"" + BatchFileType.Both + "\" />"
                + I18N.getString(locale, "batchpage;Both", new Object[] {}) + "<br/>\n");
    ...
    private static String getSubmitButton(Locale locale) {
        ...
        res.append("<input type=\"submit\" name=\"execute\" value=\""
                + I18N.getString(locale, "batchpage;Execute.batchjob", new Object[] {}) + "\"/>");
        res.append("<br/><br/>\n");
        return res.toString();
    }
    ...
    private static String getOverviewTableEntry(String batchClassPath, Locale locale) {
        ...
        try {
            ...
            File outputFile = new File(batchDir, batchName + timestamp + Constants.OUTPUT_FILE_EXTENSION);
            File errorFile = new File(batchDir, batchName + timestamp + Constants.ERROR_FILE_EXTENSION);

            // write the HTML
            res.append("    <td><a href=\"" + Constants.URL_BATCHJOB + "?" + Constants.BATCHJOB_PARAMETER + "="
                    + batchClassPath + "\">" + batchName + "</a></td>\n");
            // add time of last run
            String lastRun = "";
            if (timestamp.isEmpty()) {
                lastRun = I18N.getString(locale, "batchpage;Batchjob.has.never.been.run", new Object[] {});
            } else {
                try {
                    lastRun = new Date(Long.parseLong(timestamp.substring(1))).toString();
                } catch (NumberFormatException e) {
                    log.warn("Could not parse the timestamp '" + timestamp + "'", e);
            ...
            if (outputFile.exists() && outputFile.isFile() && outputFile.canRead()) {
                res.append("    <td><a href=" + Constants.URL_RETRIEVE_RESULT_FILES + "?filename="
                        + outputFile.getName() + ">"
                        + I18N.getString(locale, "batchpage;Download.outputfile", new Object[] {}) + "</a> "
                        + outputFile.length() + " bytes, " + FileUtils.countLines(outputFile) + " lines</td>\n");
            } else {
                res.append("    <td>" + I18N.getString(locale, "batchpage;No.outputfile", new Object[] {}) + "</td>\n");
            }
            ...
            if (errorFile.exists() && errorFile.isFile() && errorFile.canRead()) {
                res.append("    <td><a href=" + Constants.URL_RETRIEVE_RESULT_FILES + "?filename="
                        + errorFile.getName() + ">"
                        + I18N.getString(locale, "batchpage;Download.errorfile", new Object[] {}) + "</a> "
                        + errorFile.length() + " bytes, " + FileUtils.countLines(errorFile) + " lines</td>\n");
            } else {
                res.append("    <td>" + I18N.getString(locale, "batchpage;No.errorfile", new Object[] {}) + "</td>\n");
            }
        } catch (NetarkivetException e) {
            // Handle unretrievable batchjob.
            String errMsg = "Unable to instantiate '" + batchClassPath + "' as a batchjob.";
            log.warn(errMsg, e);
    ...
    public static File getBatchDir() {
        // extract the batch directory, where the old batchjobs files lies.
        File batchDir = new File(Settings.get(CommonSettings.BATCHJOBS_BASEDIR));
    ...
    private static String getArcFileForBatchjob(String classpath) throws UnknownID {
        ...
        throw new UnknownID("Unknown or undefined classpath for batchjob: '" + classpath + "'.");
    }
    ...
    public static File getJarFile(String classPath) throws ArgumentNotValid, IOFailure {
        ...
        if (!res.isFile()) {
            throw new IOFailure("The file '" + path + "' does not exist, or " + "is maybe not a file but a directory.");
        }
