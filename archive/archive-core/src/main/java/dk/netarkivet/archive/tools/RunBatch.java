    ...
    private static class RunBatchTool implements SimpleCmdlineTool {
        ...
        private boolean checkWriteFile(String fileName, String fileTag) {
            if (new File(fileName).exists()) {
                System.err.println(fileTag + " '" + fileName + "' does already exist");
                return false;
            } else {
                try {
                    ...
                    if (!tmpFile.canWrite()) {
                        System.err.println(fileTag + " '" + fileName + "' cannot be written to");
                        return false;
                    } else {
                        return true;
                    }
                } catch (IOException e) {
                    System.err.println(fileTag + " '" + fileName + "' cannot be created.");
                    return false;
                }
            }
        }
        ...
        private class BatchParameters {
            ...
            public BatchParameters() {
                ...
                options.addOption(CLASSFILE_OPTION_KEY, hasArg, "Class file to be run");
                options.addOption(JARFILE_OPTION_KEY, hasArg, "Jar file to be run (required if class file "
                        + "is in jar file)");
                options.addOption(CLASSNAME_OPTION_KEY, hasArg, "Name of the primary class to be run. Only "
                ...
                options.addOption(ERRORFILE_OPTION_KEY, hasArg, "Error file to contain errors from run "
                        + "(default is stderr)");
                options.addOption(ARGUMENTS_OPTION_KEY, hasArg, "Arguments for the batchjob. If several arguments, "
        ...
        public boolean checkArgs(String... args) {
            ...
            String classFileName = parms.cmd.getOptionValue(CLASSFILE_OPTION_KEY);
            ...
            if (classFileName != null && jars == null) {
                if (!getFileType(classFileName).equals(FileType.CLASS)) {
                    System.err.println("Argument '" + classFileName + "' is not denoting a class file");
                    return false;
                }
                if (!new File(classFileName).canRead()) {
                    System.err.println("Cannot read class file: '" + classFileName + "'");
                    return false;
                }
            }
            ...
            if (jars != null) {
                if (className == null) {
                    msg = "Using option -J also requires" + "option -N (the name of the class).";
                    System.err.println(msg);
                    return false;
                }
                ...
                for (int i = 0; i < jarList.length; i++) {
                    ...
                    if (!getFileType(jar).equals(FileType.JAR)) {
                        System.err.println("Argument '" + jar + "' is not denoting a jar file");
                        return false;
                    }
                ...
                try {
                    ...
                    System.err.println("Cannot create batchjob '" + className + "' from the jarfiles '" + jars + "'");
                    e.printStackTrace();
                    return false;
                }
            }
            ...
            if (reg != null) {
                try {
                    ...
                    System.err.println("Illegal pattern syntax: '" + reg + "'");
                    e.printStackTrace();
                    return false;
                }
            }
        ...
        private boolean isReplicaArgumentsValid() {
            ...
            if (repName != null) {
                ...
                if (!Replica.isKnownReplicaName(repName)) {
                    System.err.println("Unknown replica name '" + repName + "', known replicas are "
                            + Replica.getKnownNamesAsSet());
                    return false;
                }
                // Is it a bitarchive replica.
                if (!Replica.getReplicaFromName(repName).getType().equals(ReplicaType.BITARCHIVE)) {
                    System.err.println("Can only send a batchjob to a " + "bitarchive replica, and '"
                            + Replica.getReplicaFromName(repName) + "' is of the type '"
        ...
        public void run(String... args) {
            ...
            if (jarArgs == null) {
                LoadableFileBatchJob classJob = new LoadableFileBatchJob(new File(classFileName), argumentList);
                job = classJob;
            } else {
                ...
                job = new LoadableJarBatchJob(className, argumentList, jarFiles);
            }
            ...
            System.out.println("Running batch job '" + ((classFileName == null) ? "" : classFileName + "' ")
                    + ((jarArgs == null) ? "" : className + "' from jar-file '" + jarArgs + "' ")
                    + "on files matching '" + regexp + "' " + "on replica '" + batchReplica.getName() + "', "
                    + "output written to " + ((oFile == null) ? "stdout " : "file '" + oFile + "', ")
            ...
            if (errorFile != null) {
                try {
                    System.err.println("Writing errors to file: " + errorFile.getAbsolutePath());
            ...
            if (!exceptions.isEmpty()) {
                errorOutput.println("Failed files that produced exceptions(" + exceptions.size() + "):");
