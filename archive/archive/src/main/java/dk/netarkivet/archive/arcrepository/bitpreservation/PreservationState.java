...
public interface PreservationState {
     ...
     * This returns the status as a string for presentation purposes only. TODO
     * Needs localisation.
     *
     * @param replica
     ...
     *
     * Admin State checking: For each bitarchive the admin state is compared to
     * the checksum received from the bitarchive.
     *
     * If no checksum is received from the bitarchive the valid admin states are
     * UPLOAD_STARTED and UPLOAD_FAILED. If a checksum is received from the
     * bitarchive the valid admin state is UPLOAD_COMPLETED Admin checksum
     ...
     * returned.
     *
     * @return the name of the reference bitarchive or null if no reference
     *         exists
     */
