...
public interface BitPreservationDAO extends CleanupIF {
     ...
     * NOTE: the Checksum version of Replica must be implemented with output in
     * the same form as checksumJobOutput for implementation of bitArchive
     * replicas
     * 
     * @param checksumOutput
     *            The parsed output of a GetAllChecksumMessage as a File with
     *            ChecksumJob lines, i.e. filename##checksum.
     * @param replica
     ...
     * NOTE: the Checksum version of Replica must be implemented with output in
     * the same form as filelistJobOutput for implementation of bitArchive
     * replicas
     * 
     * @param filelistOutput
     *            A file with a list of filenames for the given replica.
     * @param replica
     *            The replica this filelist job is for.
     */
     ...
     * filelist_status = MISSING. This is done by querying the database for
     * files with no or different update date from the last known update date
     ...
     * present in the last missing files job. This is done by querying the
     * database for files with different checksum from the checksum in the last
     * known update date for bitarchive, but which are present from admin data.
     * 
     * @param replica
     *            The replica to check for.
     ...
     * @return The number of files, which does not have filelist_status =
     *         MISSING.
     */
     ...
     * @return A replica which contains the file, or null if no such replica can
     *         be found.
     */
     ...
     *            The name of the file.
     * @param badReplica
     *            A replica which is known to contain a corrupt instance of this
     *            file.
     * @return A replica which contains the file, or null if no such replica can
     *         be found.
     */
    ...
    /**
     * Method for updating the status for the files for all the replicas. If the
     * checksums of the archives differ for some replicas, then based on a
     * checksum vote, a specific checksum is chosen as the 'correct' one, and
     ...
     * as corrupt. If no winner of the voting is found, the all instances will
     * be chosen to have 'UNKNOWN' checksum status.
     * 
     * @param filename
