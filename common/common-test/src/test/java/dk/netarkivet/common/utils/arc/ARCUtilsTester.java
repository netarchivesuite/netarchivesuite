...
public class ARCUtilsTester {
    ...
    public void testInsertARCFile() throws Exception {
        ...
        FileAsserts.assertFileNumberOfLines("Should just have filedesc", outFile, TestInfo.LINES_IN_FILEDESC);
        try {
        ...
        writer = getTestARCWriter(stream, outFile);
        try {
            ARCUtils.insertARCFile(new File(TestInfo.WORKING_DIR, "does_not_exist"), writer);
        ...
        writer = getTestARCWriter(stream, outFile);
        try {
            ARCUtils.insertARCFile(new File(TestInfo.WORKING_DIR, "input-illegal.arc"), writer);
        ...
        writer.close();
        FileAsserts.assertFileNumberOfLines("Should have filedesc and entries", outFile, TestInfo.LINES_IN_FILEDESC
        ...
        writer.close();
        FileAsserts.assertFileNumberOfLines("Should have filedesc and entries", outFile, TestInfo.LINES_IN_FILEDESC
                + TestInfo.NON_FILEDESC_LINES_IN_INPUT_1 + TestInfo.NON_FILEDESC_LINES_IN_INPUT_3
                + TestInfo.NON_FILEDESC_LINES_IN_INPUT_1);

        // verify thar insertARCFile inserts correct metadata for the Records
        // contained in the ARCRecord
        stream = new PrintStream(outFile);
        writer = getTestARCWriter(stream, outFile);
        ARCUtils.insertARCFile(INPUT_3_ARC, writer);
        writer.close();
        // outFile should now contain one record with the following header-line:
        // http://www.pligtaflevering.dk/space.gif 130.226.231.6 20060203105426
        // image/gif 301
        ArchiveRecord ar = findFirstRecordWithUri(outFile, "http://www.pligtaflevering.dk/space.gif");
        if (ar == null) {
    ...
    private ArchiveRecord findFirstRecordWithUri(File f, String uri) throws IOException {
    ...
    public void testWriteFileToARC() throws Exception {
        ...
        // ensure that the date retrieved from the header is equivalent to the
