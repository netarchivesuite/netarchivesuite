...
public class ClassAsserts {
    ...
    public static <T> T assertSingleton(Class<T> c) {
        ...
        TestCase.assertSame("Expect to find only one distinct instance of " + "class " + c.getName(), o1, o2);
     ...
     */
    @SuppressWarnings("rawtypes")
    public static void assertEqualsValid(Object o1, Object o2, List excludedFields) throws IllegalAccessException {
        TestCase.fail("Unsafe to use with final fields - pending rethought");
        Class c = o1.getClass();
        TestCase.assertSame("Class must be the same for both objects", c, o2.getClass());
        ...
        for (final Field field : fields) {
            if (!excludedFields.contains(field.getName()) && !Modifier.isStatic(field.getModifiers())) {
                ...
                TestCase.assertNotSame("Values for field " + field + " should not be the same", fieldValue1,
                        fieldValue2);
        ...
        for (final Field field : fields) {
            if (!excludedFields.contains(field.getName()) && !Modifier.isStatic(field.getModifiers())) {
                TestCase.assertEquals("Objects must be same with all fields equals", o1, o2);
