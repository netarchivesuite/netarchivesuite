...
public class LogbackRecorder extends ch.qos.logback.core.AppenderBase<ch.qos.logback.classic.spi.ILoggingEvent> {
	...
	protected static final ch.qos.logback.classic.LoggerContext context = (ch.qos.logback.classic.LoggerContext)LoggerFactory.getILoggerFactory();

	protected static final ch.qos.logback.classic.Logger root = (ch.qos.logback.classic.Logger)LoggerFactory.getLogger(ch.qos.logback.classic.Logger.ROOT_LOGGER_NAME);
    ...
    public void addFilter(ch.qos.logback.core.filter.Filter<ch.qos.logback.classic.spi.ILoggingEvent> filter, String loggerName) {
    	...
    	if (logger != null) {
    		Iterator<ch.qos.logback.core.Appender<ch.qos.logback.classic.spi.ILoggingEvent>> index = logger.iteratorForAppenders();
    		while (index.hasNext()) {
	            appender = index.next();
