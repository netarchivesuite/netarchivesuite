...
public class DeDupFetchHTTP extends FetchHTTP implements AdaptiveRevisitAttributeConstants {
     ...
     * 
     * @param method
     *            HTTPMethod that allows access to the relevant HTTP header
     * @param doc
     *            The Lucene document to compare against
    ...
    /**
     * Checks if the 'etag' in the HTTP header and compares it against the etag
     * in the supplied Lucene document. If both dates are found and match then
     * the datestamp indicate non-change. Otherwise a change must be assumed.
     * 
     * @param method
     *            HTTPMethod that allows access to the relevant HTTP header
     * @param doc
     ...
     * timestamp, otherwise a random one is returned. If no hit is found null is
     * returned.
     * 
     * @param curi
     *            The CrawlURI to search for
    ...
    protected Document lookup(CrawlURI curi) {
        try {
            ...
            // query = new ConstantScoreQuery(new
            // FieldCacheTermsFilter(fieldName,
            // value));
    ...
    public void initialTasks() {
        ...
        try {
            ...
            // Reduce chunksize to avoid OOM to half the size of the default
            // (=100 MB)
