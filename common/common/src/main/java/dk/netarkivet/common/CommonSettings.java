...
public class CommonSettings {
     ...
     * The name of the environment in which this code is running. E.g. PROD,
     * RELEASETEST. It is used as a Common prefix to all JMS channels created in
     * a NetarchiveSuite installation.
     */
    public static String ENVIRONMENT_NAME = "settings.common.environmentName";

    /**
     * <b>settings.common.tempDir</b>: <br>
     * Common temporary directory for all applications. Some subdirs of this
     * directory must be set to have AllPermision in the conf/security.conf
     * file, or the web pages won't work.
     */
     ...
     * Selects the number of times the JMS connection tries to reconnect to the
     * broker.
     * */
     ...
     * dk.netarkivet.common.distribute.arcrepository.ArcRepositoryClient This
     * class will be instantiated by the ArcRepositoryClientFactory.
     */
     ...
     * implement the interface
     * dk.netarkivet.common.distribute.indexserver.JobIndexCache The class
     * instantiations are manufactored by IndexClientFactory.
     */
     ...
     * When using the Unix sort command, by default it stores temporary files in
     * the system temp dir, but if this setting is set to true, then it will be
     * configured to use the common temp dir defined by common.settings.tempDir.
     * By default the value is false (use system temp), which is the legacy
     * behavior. This setting is part of the fix for Jira issue NAS-1995.
     * 
     ...
     * The name of the directory where cache data global to the entire machine
     * can be stored. Various kinds of caches should be stored in subdirectories
     * of this.
     */
     ...
     * The number of milliseconds we wait for processes to react to shutdown
     * requests.
     */
     ...
     * The implementation class for notifications, e.g. error notification. The
     * class must extend dk.netarkivet.common.utils.Notifications
     */
     ...
     * beans. The file has a format defined by the JMX standard,
     * 
     * @see 
     ...
     * have which access privileges. The file has a format defined by the JMX
     * standard,
     * 
     * @see 
     *      <URL:http://java.sun.com/j2se/1.5.0/docs/guide/management/agent.html#
     ...
     * The entire language setting structure under the webinterface setting. Is
     ...
     * The subclass of SiteSection that defines a part of the web interface. Is
     * part of repeated siteSection settings for each part.
     */
     ...
     * The directory or war-file containing the web application for a site
     * section. Is part of repeated siteSection settings for each part.
     */
     ...
     * The default number of jobs to show in the harvest status section, on one
     * result page.
     */
     ...
     * Valid top level domain, like .co.uk, .dk, .org. Is part of repeated in
     * settings for each top level domain
     */
     ...
     * The class that defines DB-specific methods. This class must extend the
     * DBSpecifics class
     */
     ...
     * Used for the external harvest definition database. The machine where the
     * harvest definition database is located.
     */
     ...
     * Timeout in seconds to check for the validity of a JDBC connection on the
     * server. This is the time in seconds to wait for the database operation
     ...
     * Configure the maximum size of the DB connection pool. Default value is
     * 10.
     */
     ...
     * Configure the increment size DB connection pool. Default value is 5 (half
     * the max size).
     */
     ...
     * Configure statement pooling, by setting the global maximum number of
     * pooled prepared statements for a data source. Default value is 0. Note
     * that if both {@link #DB_POOL_MAX_STM} and
     * {@link #DB_POOL_MAX_STM_PER_CONN} are set to zero, statement pooling is
     * fully deactivated.
     * 
     * @see <a href="http://www.mchange.com/projects/c3p0/index.html">c3p0
     *      documentation</a>
     */
     ...
     * Configure idle connection testing period in seconds. Default is 0, which
     * means no idle connection testing
     * 
     * @see <a href=
     ...
     * Configure if a connection validity should be checked when returned to the
     * pool. Default is false.
     * 
     * @see <a href=
     *      "http://www.mchange.com/projects/c3p0/index.html#testConnectionOnCheckin"
     *      />
     */
     ...
     * The SQL query to be used when testing an idle connection. Default is
     * empty, which means using c3p0 defaults.
     * 
     * @see <a href=
     ...
     * The entire settings for all replicas in the environment. NOTE: settings
     * for checksum replicas are not use yet
     */
    public static String REPLICAS_SETTINGS = "settings.common.replicas";

    /**
     * settings.common.replicas.<b>replica</b>: <br>
     * The path to settings belonging to an individual replica, placed under the
     * replicas setting.
     */
     ...
     * The tags for identifier of the replica, placed under the replica tag. The
     * replica id is used internally in e.g. naming of channels.
     */
    public static String REPLICAID_TAG = "replicaId";
    /**
     * settings.common.replicas.replica.<b>replicaName</b>: <br>
     * The tags for name of the replica, placed under the replica tag. The
     * replica name is used in interfaces like the GUI or command-line
     * batch-programs. The name can be the same value as the id.
     */
    public static String REPLICANAME_TAG = "replicaName";
    /**
     * settings.common.replicas.replica.<b>replicaType</b>: <br>
     * The tags for type of the replica, placed under the replica tag. The type
     * is used to identify whether it is a bitarchive or a checksum replica.
     * NOTE: checksum replicas are not implemented yet Possible values are
     * defined in ReplicaType
     */
     ...
     * Physical location of where the application is running. Only use for
     * System state GUI and deploy
     */
    public static String THIS_PHYSICAL_LOCATION = "settings.common.thisPhysicalLocation";

    /**
     * <b>settings.common.applicationName</b>: <br>
     * The name of the application, e.g. "BitarchiveServerApplication". The
     * monitor puts this with each log message.
     */
    public static String APPLICATION_NAME = "settings.common.applicationName";

    /**
     * <b>settings.common.applicationInstanceId</b>: <br>
     * The identifier of the instance of the application. This is used when
     * there are more than one of the same application running on the same
     * machine, e.g. when more harvesters are running on the same machine or
     * more bitarchive applications are running on the same machine.
     */
     ...
     * dk.netarkivet.common.utils.DefaultFreeSpaceProvider. The class must
     * implement FreeSpaceProvider-Interface.
     */
     ...
     * dk.netarkivet.archive.arcrepository.bitpreservation.ChecksumJob). Must
     * inherit FileBatchJob.
     */
     ...
     * The list of the corresponding jar-files containing the batchjob. This
     * will be used for LoadableJarBatchJobs. If no file is specified, it is
     * assumed, that the batchjob exists with the default classpath of the
     * involved applications (BitarchiveMonitor, ArcRepository, GUIWebServer and
     * BitArchive).
     */
     ...
     * The setting for the FTP data timeout in seconds. The default value is 600
     * (10 minutes).
     */
     ...
     * The name of the organization responsible. The default value is
     * netarkivet.dk
     */
