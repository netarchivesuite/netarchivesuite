...
public class WARCWriterNAS extends WARCWriter {
    ...
    protected void writeRecord(final String type, final String url, final String create14DigitDate,
        ...
        if (contentLength == 0 && (xtraHeaders == null || xtraHeaders.size() <= 0)) {
            throw new IllegalArgumentException("Cannot write record of content-length zero and base headers only.");
        }

        String header;
        try {
            header = createRecordHeader(type, url, create14DigitDate, mimetype, recordId, xtraHeaders, contentLength);

        } catch (IllegalArgumentException e) {
            logger.error("Could not write record type: {} for URL: {}", type, url, e);
            return;
        }
