...
/**
 * An iterator that filters out and converts items from another iterator. Java
 * 1.5 type: FilterIterator<T>,<S>> <S> filter(<T> o);
 * 
 * @param <T>
 *            Type of Iterator
 * @param <S>
...
public abstract class FilterIterator<T, S> implements Iterator<S> {
    ...
    /**
     * Create a new iterator based on an old one. The old one must not contain
     * any null entries.
     *
     * @param i
     *            An iterator
