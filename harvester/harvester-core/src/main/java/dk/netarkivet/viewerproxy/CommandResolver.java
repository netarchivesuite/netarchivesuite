 ...
 * (http://<<serverhost>>/<<command>>/<<param>>=<<value>>*).
 */
...
public abstract class CommandResolver implements URIResolverHandler, URIResolver {
     ...
     *
     * @param ur The URIResolver that handles URIs that are not handled by this resolver.
     */
    ...
    /**
     * Abstract method for parsing of the URL and delegating to relevant methods. This should start by calling
    ...
    /**
     * Returns true if the request specifies the host that we're running on. Alternative specifications such as
     * 'localhost' or '127.0.0.1' or an actual IP of this machine are not considered command hosts. Local hosts would
     * only be command hosts if we happen to get a request from a browser running on this machine.
     *
     * @param request An HTTP request.
     * @return True if the HTTP request specifies a URI that indicates this machine, false otherwise.
     */
    protected static boolean isCommandHostRequest(Request request) {
        return request != null && request.getURI() != null && request.getURI().getHost() != null
                && request.getURI().getHost().equals(VIEWERPROXY_COMMAND_NAME);
    }
}
