...
<%
    ...
    try {
        HTMLUtils.forwardOnMissingParameter(pageContext, Constants.JOB_PARAM);
        regexp = request.getParameter(Constants.REGEXP_PARAM);
        domain = request.getParameter(Constants.DOMAIN_SEARCH_PARAM);
        if ( (regexp == null || regexp.length() == 0) && (domain == null || domain.length() == 0) ) {
            pageContext.getRequest().setAttribute("message", "Must specify either 'domain' or 'regexp' " + "parameter on request.");
            RequestDispatcher rd = pageContext.getServletContext().getRequestDispatcher("/message.jsp");
        ...
        int maxLinesInBrowser = Settings.getInt(HarvesterSettings.MAX_CRAWLLOG_IN_BROWSER);
...
<h3>
	<fmt:message
		key="pagetitle;qa.crawllog.lines.for.job.0.matching.regexp.1">
...
<% } else { %>
<h3>
	<fmt:message key="pagetitle;qa.crawllog.lines.for.domain.0.in.1">
