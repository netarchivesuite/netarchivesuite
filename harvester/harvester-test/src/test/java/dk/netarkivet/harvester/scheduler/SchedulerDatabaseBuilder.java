...
/**
 * This will add the components to an empty NetarchiveSuite database that is
 * necessary for the two unit tests in the SchedulerTesterSuite.
 * 
 * Add domain netarkivet.dk with two seedlist Add the three schedules Dagligt,
 * Once_a_day, OnceOnly Add one selective harvestdefinition (isActive=true)
 */
public class SchedulerDatabaseBuilder {
    ...
    private void close() throws Exception {
        ...
        File destinationDir = new File(TestInfo.ORIGINALS_DIR, "fullhddb-" + System.currentTimeMillis());
        destinationDir.mkdir();
        System.out.println("Copied database to " + destinationDir.getAbsolutePath());
        TestFileUtils.copyDirectoryNonCVS(new File(TestInfo.WORKING_DIR, "fullhddb"), destinationDir);
        FileUtils.removeRecursively(TestInfo.WORKING_DIR);
    ...
    private void doWork() {
        ...
        SeedList englishSeed = new SeedList("english", "http://www.netarkivet.dk/index-en.htm/n");
        SeedList danishSeed = new SeedList("danish", "http://www.netarkivet.dk/website/links/index-da.htm/n");

        d.addSeedList(englishSeed);
        d.addSeedList(danishSeed);

        DomainConfiguration cfg1 = new DomainConfiguration("Dansk_netarkiv_fuld_dybde", d, Arrays.asList(danishSeed),
                new ArrayList<Password>());
        cfg1.setMaxBytes(dk.netarkivet.harvester.datamodel.Constants.DEFAULT_MAX_BYTES);
        cfg1.setOrderXmlName("FullSite-order");
        d.addConfiguration(cfg1);

        DomainConfiguration cfg2 = new DomainConfiguration("Engelsk_netarkiv_et_niveau", d, Arrays.asList(englishSeed),
                new ArrayList<Password>());
        cfg2.setMaxBytes(dk.netarkivet.harvester.datamodel.Constants.DEFAULT_MAX_BYTES);
        ...
        PartialHarvest ph = PartialHarvest.createPartialHarvest(dcs, s2, "Testhøstning", "No Comments", "Everybody");
        ph.setActive(true);
