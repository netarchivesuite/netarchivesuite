...
public class DomainDAOTester extends DataModelTestCase {
    ...
    public void testOptimisticLocking() {
        ...
        assertEquals("The instances should have the same edition number", inst2.getEdition(), inst1.getEdition());
    ...
    public void testGetAllDomains() {
        ...
        assertEquals(NUM_DOMAINS + " domains added, " + NUM_DOMAINS + " domains expected.\nOriginal domains: "
                + domains + "\nNew domains: " + newDomains, NUM_DOMAINS, added_domains);
    }
    ...
    public void testRead() throws Exception {
        ...
        assertEquals("Default configuration should be correct", "fuld_dybde", d.getDefaultConfiguration().getName());
        int count = 0;
        ...
        assertFalse("Number of password entries should be 0", d.getAllPasswords().hasNext());
        assertEquals("Amount of domain owner info should be correct", 1, d.getAllDomainOwnerInfo().length);
        count = 0;
        for (Iterator<SeedList> i = d.getAllSeedLists(); i.hasNext(); i.next(), count++) {
        }
        assertEquals("Number of seedlists should be 1", 1, count);
        assertEquals("Amount of owner info should be 1", 1, d.getAllDomainOwnerInfo().length);
    }
    ...
    public void testReadAndWriteHarvestInfo() throws Exception {
        ...
        HarvestInfo[] his = new HarvestInfo[] {
                new HarvestInfo(Long.valueOf(4), domain0.getName(), "fuld_dybde", new Date(400000L), 40, 1,
                        StopReason.DOWNLOAD_COMPLETE),
                new HarvestInfo(Long.valueOf(2), domain0.getName(), "fuld_dybde", new Date(300000L), 30, 2,
        ...
        assertFalse("There should be 0 harvest infos before adding", domainHistory0.getHarvestInfo().hasNext());
        ...
        HarvestInfo[] readhis = readhislist.toArray(new HarvestInfo[0]);

        assertTrue("The info read should be the same as the info stored:" + Arrays.asList(his) + "\n" + "but was: "
                + Arrays.asList(readhis), Arrays.equals(his, readhis));
    }
    ...
    public void testGetHarvestInfoBasedOnPreviousHarvestDefinition() throws Exception {
        ...
        // An older harvest info with same harvest definition that should NOT be
        // returned
        // HarvestInfo old_hi1 = new HarvestInfo(hd.getOid(), domain1.getName(),
        // config1.getName(), then);
        ...
        HarvestInfo hi0 = new HarvestInfo(hd.getOid(), domain0.getName(), config0.getName(), now, 1L, 1L,
                StopReason.OBJECT_LIMIT);
        domain0.getHistory().addHarvestInfo(hi0);
        dao.update(domain0);

        HarvestInfo hi1 = new HarvestInfo(hd.getOid(), domain1.getName(), config1.getName(), now, 1L, 1L,
                StopReason.OBJECT_LIMIT);
        domain1.getHistory().addHarvestInfo(hi1);
        dao.update(domain1);

        HarvestInfo hi2 = new HarvestInfo(hd.getOid(), domain2.getName(), config2.getName(), now, 1L, 1L,
                StopReason.DOWNLOAD_COMPLETE);
        ...
        HarvestInfo new_hi0 = new HarvestInfo(Long.valueOf(43L), domain0.getName(), config0.getName(), later, 1L, 1L,
                StopReason.DOWNLOAD_COMPLETE);
    ...
    public void testGetDomainHarvestInfo() throws Exception {
        ...
        HarvestInfo hi = new HarvestInfo(42L, 1L, domainName, dc.getName(), new Date(2L), 10000L, 100L,
                StopReason.DOWNLOAD_COMPLETE);
    ...
    public void testDeleteSubparts() {
        ...
        assertFalse("Password should be deleted in stored version", d.hasPassword("alphapassword"));
        ...
        dao.update(d);
        d = dao.read(domainName);
        assertFalse("Seedlist should be deleted in stored version", d.hasSeedList("deletable"));
        ...
        try {
            ...
            StringAsserts.assertStringContains("Should mention config in error message", d.getDefaultConfiguration()
                    .getName(), e.getMessage());
        }
        d = dao.read(domainName);
        assertTrue("Seedlist should still exist in stored version", d.hasSeedList("default"));
        ...
        d = dao.read(domainName);
        assertTrue("Default config should still exist in stored version", d.hasConfiguration(defaultConfigName));
        ...
        dao.update(d);
        d = dao.read(domainName);
        assertFalse("Deleted config should be gone from domain", d.hasConfiguration(nonDefaultConfigName));
    }
    ...
    public void testInsertOwnerInfo() {
        ...
        Domain d = dao.read(domainName);
        assertTrue("Domain should not have ownerinfo before insertion", d.getAllDomainOwnerInfo().length == 0);
        ...
        assertEquals("Should have one ownerinfo after insertion", 1, info.length);
        ...
        assertEquals("Should have two ownerinfo after insertion", 2, info.length);
    }
    ...
    public void testInsertPassword() {
        ...
        Domain d = dao.read(domainName);
        assertEquals("Domain should not have passwords before insertion", 0, IteratorUtils.toList(d.getAllPasswords())
                .size());
        d.addPassword(new Password("foo", "bar", "baz", "qux", "quux", "quuux"));
        dao.update(d);
        Domain d2 = dao.read(d.getName());
        List<Password> info = IteratorUtils.toList(d2.getAllPasswords());
        assertEquals("Should have one password after insertion", 1, info.size());
    ...
    public void testReadSparse() {
        ...
        assertEquals("Name should be right", domainName, d.getName());
        Iterator<String> domainConfigurationNames = d.getDomainConfigurationNames().iterator();
        assertTrue("Should have configs", domainConfigurationNames.hasNext());
        assertEquals("Should have right config", "fuld_dybde", domainConfigurationNames.next());
        assertFalse("Should have no more configs", domainConfigurationNames.hasNext());
    }
    ...
    public void testDomainHarvestInfoConstructor() {
        ...
        DomainHarvestInfo dhi = new DomainHarvestInfo(domain, jobId, harvestName, harvestId, harvestNum, configName,
                startDate, endDate, bytesDownloaded, docsdownloaded, theReason);
