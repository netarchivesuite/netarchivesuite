...
public class HarvestDefinitionTester extends DataModelTestCase {
    ...
    public void testValidityOfConstructorArguments() {
        ...
        try {
            ...
            HarvestDefinition.createPartialHarvest(null, schedule, TestInfo.DEFAULT_HARVEST_NAME,
                    TestInfo.DEFAULT_HARVEST_COMMENT, TestInfo.DEFAULT_HARVEST_AUDIENCE);
        ...
        try {
            HarvestDefinition.createPartialHarvest(domainConfigs, null, TestInfo.DEFAULT_HARVEST_NAME,
                    TestInfo.DEFAULT_HARVEST_COMMENT, TestInfo.DEFAULT_HARVEST_AUDIENCE);
        ...
        Schedule unknownSchedule = Schedule.getInstance(null, null, new HourlyFrequency(1), "UnknownSchedule", "");
        try {
            HarvestDefinition.createPartialHarvest(domainConfigs, unknownSchedule, TestInfo.DEFAULT_HARVEST_NAME,
                    TestInfo.DEFAULT_HARVEST_COMMENT, TestInfo.DEFAULT_HARVEST_AUDIENCE);
        ...
        // Ad. 3:
        try {
            HarvestDefinition.createPartialHarvest(domainConfigs, schedule, null, TestInfo.DEFAULT_HARVEST_COMMENT,
        ...
        try {
            HarvestDefinition.createPartialHarvest(domainConfigs, schedule, "", TestInfo.DEFAULT_HARVEST_COMMENT,
                    TestInfo.DEFAULT_HARVEST_AUDIENCE);
        ...
        // Ad. 4:
        try {
            HarvestDefinition.createPartialHarvest(domainConfigs, schedule, TestInfo.DEFAULT_HARVEST_NAME, null,
                    TestInfo.DEFAULT_HARVEST_AUDIENCE);
    ...
    public void testValidityOfArgumentsNextDate() {
        ...
        PartialHarvest harvestDef = HarvestDefinition.createPartialHarvest(domainConfigs, schedule,
                TestInfo.DEFAULT_HARVEST_NAME, TestInfo.DEFAULT_HARVEST_COMMENT, TestInfo.DEFAULT_HARVEST_AUDIENCE);
        try {
    ...
    public void unstableTestSetAndGet() {
        ...
        assertEquals("Submission date value set expected", date, harvestDef.getSubmissionDate());

        harvestDef.setNumEvents(42);
        assertEquals("Number of events getter should get what I set", 42, harvestDef.getNumEvents());

        harvestDef.setNextDate(date);
        assertEquals("Next date getter should get what I set", date, harvestDef.getNextDate());
    }
    ...
    public void testSetConfigurations() {
        ...
        Object o2 = l2.next();
        assertTrue("Did not get the same objects back", (cfg2 == o1 && cfg3 == o2) || (cfg2 == o2 && cfg3 == o1));
    ...
    public void testRunNowPartialHarvest() {
        ...
        PartialHarvest harvestDef = HarvestDefinition.createPartialHarvest(domainConfigs, schedule,
                TestInfo.DEFAULT_HARVEST_NAME, TestInfo.DEFAULT_HARVEST_COMMENT, TestInfo.DEFAULT_HARVEST_AUDIENCE);
        Calendar cal = new GregorianCalendar(2005, Calendar.FEBRUARY, 21, 12, 0, 0);
        ...
        assertTrue("Should start now if nextDate is now", harvestDef.runNow(testDate));
        assertTrue("Should start now if now is after nextDate", harvestDef.runNow(afterDate));
        assertFalse("Should not start now if now is before nextDate", harvestDef.runNow(beforeDate));
    ...
    public void testRunNowFullHarvest() {
        ...
        FullHarvest harvestDef = HarvestDefinition.createFullHarvest(TestInfo.DEFAULT_HARVEST_NAME,
        ...
        assertTrue("Before creating any jobs, runNow() should be true", harvestDef.runNow(new Date()));
    ...
    public void testCreateJobsUpdatesEventFields() {
        ...
        PartialHarvest harvestDef0 = HarvestDefinition.createPartialHarvest(domainConfigs, sched0,
                TestInfo.DEFAULT_HARVEST_NAME + "1", TestInfo.DEFAULT_HARVEST_COMMENT,
                TestInfo.DEFAULT_HARVEST_AUDIENCE);
        harvestDef0.setSubmissionDate(new Date());

        PartialHarvest harvestDef1 = HarvestDefinition.createPartialHarvest(domainConfigs, sched1,
                TestInfo.DEFAULT_HARVEST_NAME + "2", TestInfo.DEFAULT_HARVEST_COMMENT,
                TestInfo.DEFAULT_HARVEST_AUDIENCE);
        ...
        PartialHarvest harvestDef2 = HarvestDefinition.createPartialHarvest(domainConfigs, sched2,
                TestInfo.DEFAULT_HARVEST_NAME + "3", TestInfo.DEFAULT_HARVEST_COMMENT,
                TestInfo.DEFAULT_HARVEST_AUDIENCE);
        ...
        assertEquals("Must count up number of events on generating jobs", numEvents1 + 1, harvestDef1.getNumEvents());
        assertEquals("No more events, schedule time ended", null, harvestDef1.getNextDate());

        int numEvents2 = harvestDef2.getNumEvents();
        generateJobs(harvestDef2);
        assertEquals("Must count up number of events on generating jobs", numEvents2 + 1, harvestDef2.getNumEvents());
        assertEquals("No more events, just one harvest", null, harvestDef2.getNextDate());
    }
    ...
    public void testCreateJobsSkipsEvents() {
        ...
        TimedSchedule hourlySchedule = new TimedSchedule(null, null, new HourlyFrequency(1), "hourly", "");
        ...
        List<DomainConfiguration> dc = Collections.singletonList(defaultDomain.getDefaultConfiguration());
        PartialHarvest hd = HarvestDefinition.createPartialHarvest(dc, hourlySchedule, "hd", "",
                TestInfo.DEFAULT_HARVEST_AUDIENCE);
        ...
        assertTrue("Next date should be in the future", hd.getNextDate().after(now));
    ...
    public void testSkipInactive() {
        ...
        // A harvest definition with an hourly schedule and next date set to
        // three hours ago
        List<DomainConfiguration> dc = Collections.singletonList(TestInfo.getDefaultDomain().getDefaultConfiguration());
        PartialHarvest hd = HarvestDefinition.createPartialHarvest(dc, hourlySchedule, "hd", "",
                TestInfo.DEFAULT_HARVEST_AUDIENCE);
        hd.setNextDate(threeHoursAgo.getTime());
        hd.setSubmissionDate(new Date());
    ...
    public void testReset() {
        ...
        schedule = Schedule.getInstance(first, null, new DailyFrequency(1), "test", "");
    ...
    public void testCreateJobsByOrderXml() {
        ...
        Settings.set(HarvesterSettings.JOBS_MAX_RELATIVE_SIZE_DIFFERENCE, "50");
        Settings.set(HarvesterSettings.JOBS_MAX_TOTAL_JOBSIZE, "40000");
        Settings.set(HarvesterSettings.JOBS_MIN_ABSOLUTE_SIZE_DIFFERENCE, "10000");
        int jobsMade = generateJobs(hd);
    ...
    public void testCreateIncrementalSnapShot() {
        ...
        HarvestInfo old_hi0 = new HarvestInfo(Long.valueOf(42L), domain0.getName(), config0.getName(), then, 1L, 1L,
        ...
        HarvestInfo old_hi1 = new HarvestInfo(hdOID, domain1.getName(), config1.getName(), then, 1L, 1L,
                StopReason.DOWNLOAD_COMPLETE);
        ...
        HarvestInfo hi0 = new HarvestInfo(hdOID, domain0.getName(), config0.getName(), now, 1L, 1L,
                StopReason.OBJECT_LIMIT);
        domain0.getHistory().addHarvestInfo(hi0);
        dao.update(domain0);

        HarvestInfo hi1 = new HarvestInfo(hdOID, domain1.getName(), config1.getName(), now, 1L, 1L,
                StopReason.SIZE_LIMIT);
        domain1.getHistory().addHarvestInfo(hi1);
        dao.update(domain1);

        HarvestInfo hi2 = new HarvestInfo(hdOID, domain2.getName(), config2.getName(), now, 1L, 1L,
                StopReason.DOWNLOAD_COMPLETE);
        domain2.getHistory().addHarvestInfo(hi2);
        dao.update(domain2);

        HarvestInfo hi3 = new HarvestInfo(hdOID, domain3.getName(), config3.getName(), now, 1L, 1L,
        ...
        HarvestInfo newHi0 = new HarvestInfo(Long.valueOf(43L), domain0.getName(), config0.getName(), later, 1L, 1L,
                StopReason.DOWNLOAD_COMPLETE);
        ...
        FullHarvest fh = HarvestDefinition.createFullHarvest("Test incremental", "Just a test", hd.getOid(), 9999,
                Constants.DEFAULT_MAX_BYTES, Constants.DEFAULT_MAX_JOB_RUNNING_TIME);
    ...
    public void testCreateFullHarvest() {
        ...
        assertEquals("HD read back should be the same, even with cycles", hd1, hd1b);
    }
    ...
    public void testCompareConfigsDesc() throws NoSuchFieldException, IllegalAccessException,
        ...
        d.addConfiguration(cfg1);
        DomainConfiguration cfg2 = new DomainConfiguration("config2", d, seedlists, new ArrayList<Password>());
        cfg2.setMaxBytes(3000000);
        cfg2.setOrderXmlName("X");
        d.addConfiguration(cfg2);
        DomainConfiguration cfg3 = new DomainConfiguration("config3", d, seedlists, new ArrayList<Password>());
        cfg3.setMaxBytes(2000000);
        cfg3.setOrderXmlName("Y");
        d.addConfiguration(cfg3);
        DomainConfiguration cfg4 = new DomainConfiguration("config4", d, seedlists, new ArrayList<Password>());
        cfg4.setMaxBytes(2000000);
        cfg4.setOrderXmlName("X");
        d.addConfiguration(cfg4);
        DomainConfiguration cfg5 = new DomainConfiguration("config5", d, seedlists, new ArrayList<Password>());
        cfg5.setMaxBytes(1000000);
        cfg5.setOrderXmlName("Y");
        d.addConfiguration(cfg5);
        DomainConfiguration cfg6 = new DomainConfiguration("config6", d, seedlists, new ArrayList<Password>());
        ...
        assertNotNull("If this fails we no longer have the comparator as inner " + "class."
                + " In that case update this unit test to make sure we" + " get the CompareConfigDesc comparator.", c);
        Constructor declaredConstructor = c.getDeclaredConstructor(long.class, long.class);

        // Make a comparator where the deciding limit is always the domain
        // config
        Comparator<DomainConfiguration> compareconfigdesc = (Comparator<DomainConfiguration>) declaredConstructor
                .newInstance(-1, 4000000L);
        Collections.sort(list, compareconfigdesc);

        // Test order
        assertEquals("First order template with highest byte limit expected", cfg2, list.get(0));
        assertEquals("First order template with second byte limit expected", cfg4, list.get(1));
        assertEquals("First order template with lowest byte limit expected", cfg1, list.get(2));
        assertEquals("Second order template with highest byte limit expected", cfg6, list.get(3));
        assertEquals("Second order template with second byte limit expected", cfg3, list.get(4));
        ...
        // harvest limit
        compareconfigdesc = (Comparator<DomainConfiguration>) declaredConstructor.newInstance(
                Constants.HERITRIX_MAXOBJECTS_INFINITY, 1500000L);

        Collections.sort(list, compareconfigdesc);

        // Test order
        assertEquals("First order template with first harvest limit expected", cfg2, list.get(0));
        assertEquals("First order template with second harvest limit expected", cfg4, list.get(1));
        assertEquals("First order template with lowest byte limit expected", cfg1, list.get(2));
        assertEquals("Second order template with first harvest limit expected", cfg3, list.get(3));
        assertEquals("Second order template with second harvest limit expected", cfg6, list.get(4));
        assertEquals("Second order template with lowest byte limit expected", cfg5, list.get(5));

        // Tweak expected number in the configurations by adding harvest info
        d.getHistory().addHarvestInfo(
                new HarvestInfo(42L, 1L, d.getName(), cfg2.getName(), new Date(), 1000000, 2000,
                        StopReason.DOWNLOAD_COMPLETE));
        d.getHistory().addHarvestInfo(
                new HarvestInfo(42L, 1L, d.getName(), cfg3.getName(), new Date(), 1000000, 3000,
                        StopReason.DOWNLOAD_COMPLETE));
        d.getHistory().addHarvestInfo(
                new HarvestInfo(42L, 1L, d.getName(), cfg4.getName(), new Date(), 1000000, 4000,
                        StopReason.DOWNLOAD_COMPLETE));
        d.getHistory().addHarvestInfo(
                new HarvestInfo(42L, 1L, d.getName(), cfg1.getName(), new Date(), 1000000, 1000,
                        StopReason.DOWNLOAD_COMPLETE));
        d.getHistory().addHarvestInfo(
                new HarvestInfo(42L, 1L, d.getName(), cfg5.getName(), new Date(), 1000000, 5000,
                        StopReason.DOWNLOAD_COMPLETE));
        d.getHistory().addHarvestInfo(
                new HarvestInfo(42L, 1L, d.getName(), cfg6.getName(), new Date(), 1000000, 6000,
                        StopReason.DOWNLOAD_COMPLETE));
        ...
        assertEquals("First order template with highest byte limit expected", cfg2, list.get(0));
        assertEquals("First order template with low byte limit and high " + "expectation expected", cfg4, list.get(1));
        assertEquals("First order template with low byte limit and low " + "expectation expected", cfg1, list.get(2));
        assertEquals("Second order template with highest byte limit expected", cfg6, list.get(3));
        assertEquals("Second order template with low byte limit and high " + "expectation expected", cfg5, list.get(4));
    ...
    public void testEquals() {
        PartialHarvest harvestDef1 = HarvestDefinition.createPartialHarvest(
                Arrays.asList(new DomainConfiguration[] {}), schedule, TestInfo.DEFAULT_HARVEST_NAME,
                TestInfo.DEFAULT_HARVEST_COMMENT, TestInfo.DEFAULT_HARVEST_AUDIENCE);

        PartialHarvest harvestDef2 = HarvestDefinition.createPartialHarvest(
                Arrays.asList(new DomainConfiguration[] {}), schedule, TestInfo.DEFAULT_HARVEST_NAME,
                TestInfo.DEFAULT_HARVEST_COMMENT, TestInfo.DEFAULT_HARVEST_AUDIENCE);
