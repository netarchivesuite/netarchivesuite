...
public class DomainConfigurationTester extends DataModelTestCase {
    ...
    public void testConstructorAndSomeGetters() {
        ...
        assertEquals("Expecting name that we created the object with", TestInfo.CONFIGURATION_NAME, cfg.getName());
        assertEquals("Expecting domain that we created the object with", wd.getName(), cfg.getDomainName());
        Iterator<SeedList> seeds = cfg.getSeedLists();
        assertEquals("Expecting the seedlists that we created the object with", TestInfo.seedlist, seeds.next());
        assertEquals("should have just one seedlist", false, seeds.hasNext());
        assertEquals("Expecting empty list of passwords that we created the" + " object with", false, cfg
                .getPasswords().hasNext());
    }
    ...
    public void testMaxBytes() {
        ...
        // Retrieve the configuration in non-trivial way
        DomainConfiguration cfg = wd.getDefaultConfiguration();
        assertEquals("Expecting same maxBytes as stored", maxBytes, cfg.getMaxBytes());
        ...
        assertEquals("Expecting same maxBytes as stored", maxBytes2, cfg2.getMaxBytes());
    ...
    public void testRemovePassword() throws Exception {
        ...
        assertFalse("Configuration should have password removed", conf.usesPassword(TestInfo.PASSWORD_NAME));
        try {
            conf.removePassword(TestInfo.PASSWORD_NAME);
    ...
    public void testAddSeedlist() {
        ...
        assertSame("Should have domains seedlist, not the copy", seedlist, dc.getSeedLists().next());
        try {
    ...
    public void testAddPassword() {
        ...
        assertSame("Should have domains password, not the copy", password, dc.getPasswords().next());
        try {
            copy = new Password("badname", password.getComments(), password.getPasswordDomain(), password.getRealm(),
    ...
    private void addHistoryObject(Domain d, DomainConfiguration dc, Date d1, final long countObjectRetrieved,
            long sizeDataRetrieved, final StopReason stopReason) {
        d.getHistory().addHarvestInfo(
                new HarvestInfo(Long.valueOf(1L), d.getName(), dc.getName(), d1, sizeDataRetrieved,
