...
public class MultiFileBasedCacheTester {
    ...
    public void testGetCacheFile() throws Exception {
        ...
        assertEquals("Expect value on 1 id", "1-cache", multiFileBasedCache.getCacheFile(
                new HashSet<Long>(Arrays.asList(new Long[] {1L}))).getName());
        assertEquals("Expect value on 2 ids", "1-2-cache", multiFileBasedCache.getCacheFile(
                new HashSet<Long>(Arrays.asList(new Long[] {1L, 2L}))).getName());
