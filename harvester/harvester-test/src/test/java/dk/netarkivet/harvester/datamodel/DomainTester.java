...
public class DomainTester extends DataModelTestCase {
    ...
    public void testExtendedFields() {
        ...
        ExtendedField extField = new ExtendedField(null, (long) ExtendedFieldTypes.DOMAIN, "Test", "12345",
                ExtendedFieldDataTypes.STRING, true, 1, "defaultvalue", "", ExtendedFieldConstants.MAXLEN_EXTF_NAME);
        extDAO.create(extField);

        extField = new ExtendedField(null, (long) ExtendedFieldTypes.DOMAIN, "Boolean", "",
                ExtendedFieldDataTypes.BOOLEAN, true, 1, "true", "", ExtendedFieldConstants.MAXLEN_EXTF_BOOLEAN);
        extDAO.create(extField);
    ...
    public void testExtendedFields2() {
        ExtendedFieldDAO extDAO = ExtendedFieldDBDAO.getInstance();
        ExtendedField extField = new ExtendedField(null, (long) ExtendedFieldTypes.DOMAIN, "Test", "0000",
                ExtendedFieldDataTypes.NUMBER, false, 1, "", "", ExtendedFieldConstants.MAXLEN_EXTF_NAME);
        extDAO.create(extField);
    ...
    public void testSetAndGet() {
        ...
        assertEquals("Expecting name we asked for", TestInfo.DOMAIN_NAME, wd.getName());
    ...
    public void testRemoveConfiguration() {
        ...
        wd.addConfiguration(cfg2);
        dao.create(wd);
        String defaultcfgName = Settings.get(HarvesterSettings.DOMAIN_DEFAULT_CONFIG);
        assertEquals("The default configuration does not change when adding new configs", wd.getDefaultConfiguration()
                .getName(), defaultcfgName);
        ...
        assertEquals("The first configuration added becomes the default", wd.getDefaultConfiguration().getName(),
                defaultcfgName);
        wd.setDefaultConfiguration("Another");
        dao.update(wd);
        assertEquals("New default", wd.getDefaultConfiguration().getName(), "Another");
        wd.removeConfiguration(defaultcfgName);
    }
    ...
    public void testUpdateConfiguration() throws Exception {
        ...
        assertFalse("New and old config should not be the same", conf.equals(conf3));
    }
    ...
    public void testAddSeedList() {
        Domain d = Domain.getDefaultDomain(TestInfo.DEFAULTNEWDOMAINNAME);
        assertFalse("New seedlist must not exist beforehand", d.hasSeedList(TestInfo.SEEDLISTNAME));
        d.addSeedList(new SeedList(TestInfo.SEEDLISTNAME, TestInfo.SEEDS1));
        assertTrue("New seedlist should exist afterwards", d.hasSeedList(TestInfo.SEEDLISTNAME));
        ...
        try {
            d.addSeedList(new SeedList(TestInfo.SEEDLISTNAME, TestInfo.SEEDS2));
    ...
    public void testUpdateSeedList() {
        Domain d = Domain.getDefaultDomain(TestInfo.DEFAULTNEWDOMAINNAME);
        try {
            d.updateSeedList(new SeedList(TestInfo.SEEDLISTNAME, TestInfo.SEEDS1));
        ...
        d.updateSeedList(new SeedList(TestInfo.SEEDLISTNAME, TestInfo.SEEDS2));
        ...
        assertFalse("Domain should not have unknown seedlist", d.hasSeedList(TestInfo.SEEDLISTNAME_JOB4));
    ...
    public void testGetAllConfigurations() {
        ...
        assertFalse("The configs must be different", cfgA.getName().equals(cfgB.getName()));
    }
    ...
    public void testGetHistoricalData() {
        ...
        HarvestInfo harvestinfo = new HarvestInfo(harvestID, "bar", "foo", date, 42, 1, StopReason.DOWNLOAD_COMPLETE);
        domainHistory.addHarvestInfo(harvestinfo);

        assertNotNull("expect not null", domainHistory.getHarvestInfo());
        assertTrue("at least one info expected", domainHistory.getHarvestInfo().hasNext());

        assertEquals("Expecting prev. set size", 42, (domainHistory.getHarvestInfo().next()).getSizeDataRetrieved());
    }
    ...
    public void testAddHarvestInfo() {
        ...
        assertEquals("Should have the same date", harvesInfo_2.getDate(), date);
        assertEquals("Should have the same harvestID", harvesInfo_2.getHarvestID(), harvestID);
    }
    ...
    public void testSetStopReason() {
        ...
        assertEquals("Output should give the same", StopReason.DOWNLOAD_COMPLETE, harvestinfor.getStopReason());
    }
    ...
    public void testSetCountObjectRetrieved() {
        ...
        HarvestInfo harvestinfor = new HarvestInfo(harvestID, "bar", "foo", date, 1L, 1000L,
                StopReason.DOWNLOAD_COMPLETE);
        assertEquals("should give the same", 1000L, harvestinfor.getCountObjectRetrieved());
    }
    ...
    public void testSetSizeDataRetrieved() {
        ...
        HarvestInfo harvestinfor = new HarvestInfo(harvestID, "bar", "foo", date, 1000L, 1L,
                StopReason.DOWNLOAD_COMPLETE);

        assertEquals("Should be the same", 1000L, harvestinfor.getSizeDataRetrieved());
    }
    ...
    public void testLoadAndVerifyAllDomainCreated() {
        ...
        ingestDList.updateDomainInfo(domainfile, null, new Locale("en"));

        assertTrue("Domains should exist after updating domain info", dao.exists("jp.dk"));
    ...
    public void testSetAndGetInvalidValues() {
        ...
        try {
            new HarvestInfo(defaultHid, "foo", "bar", null, 1L, 1L, StopReason.DOWNLOAD_COMPLETE);
            fail("parameters should not be null");
        } catch (ArgumentNotValid e) {
            // expected
        }
        ...
        try {
            new HarvestInfo(defaultHid, "foo", "bar", date, -1L, 1L, StopReason.DOWNLOAD_COMPLETE);
    ...
    public void testGetDefaultDomain() throws Exception {
        ...
        assertEquals("Domain name for default domain should be right", "foo.dk", d.getName());
        DomainConfiguration conf = d.getDefaultConfiguration();
        assertNotNull("Configuration for default domain should not be nulL", conf);
    ...
    public void testUpdatePassword() throws Exception {
        ...
        Password p2 = new Password(TestInfo.PASSWORD_NAME, "Secret comment!", "www.snort.dk", "backdoor", "cracker",
                "letmein");
        d.updatePassword(p2);
        assertEquals("New password should be found instead of old", p2, d.getPassword(TestInfo.PASSWORD_NAME));
        assertFalse("Old and new password should not be the same", p2.equals(TestInfo.password));
    }
    ...
    public void testRemovePassword() throws Exception {
        ...
        d.removePassword(TestInfo.PASSWORD_NAME);
        assertFalse("Password should be gone after removePassword", d.hasPassword(TestInfo.PASSWORD_NAME));
        try {
            d.removePassword(TestInfo.PASSWORD_NAME);
    ...
    public void testSetCrawlerTraps() {
        ...
        assertEquals("Crawler traps should be remembered as given", definedregexps, foundregexps);

        // Test that a crawlertrap-regexp only consisting of whitespace is not
        // considered
        ...
        definedregexps.add(" ");
        d.setCrawlerTraps(definedregexps, strictMode);
        assertEquals("Crawler traps containing only whitespace should not be considered as a valid crawler-trap", 0, d
                .getCrawlerTraps().size());

        // Whitespace is not removed from a crawler-trap containing other
        ...
        assertEquals("Leading and trailing whitespace should be conserved in a regexp containing other "
                + "characters than whitespace", definedregexps, d.getCrawlerTraps());
    ...
    public void testAliasGettersAndSetters() {
        ...
        assertEquals("Should have set alias after rawSetAliasInfo", "fnord.dk", d.getAliasInfo().getAliasOf());
        assertEquals("Should have set lastUpdate to given date", date, d.getAliasInfo().getLastChange());
        // Check that setting alias to the same as before does not
        // force an update of lastAliasUpdate
        d.updateAlias("fnord.dk");
        assertNotNull("Should have updated lastUpdate on non-changing set", d.getAliasInfo().getLastChange());
        ...
        Date after = new Date();
        assertFalse("Should have lastUpdate set within time being set", d.getAliasInfo().getLastChange().after(after)
        ...
        assertEquals("Should have alias unchanged after error", "kb.dk", d.getAliasInfo().getAliasOf());
        assertEquals("Should have lastAliasUpdate unchanged after error", setTo, d.getAliasInfo().getLastChange());
        ...
        try {
            ...
            fail("Should not allow transitive (to aliased) alias - " + d + " is aliased to "
                    + d.getAliasInfo().getAliasOf());
        ...
        try {
            ...
            fail("Should not allow transitive alias (from father) - " + dao.getAliases(d3.getName())
                    + " are aliases of " + d3);
        } catch (IllegalState e) {
            // Expected
        }
        ...
        assertEquals("Domain should set other alias", "statsbiblioteket.dk", d.getAliasInfo().getAliasOf());
        assertTrue("Should have lastUpdate set within time being set", d.getAliasInfo().getLastChange().after(after));
    ...
    public void testGetSortedSeedlistAndDomainConfigurationsAndPasswords() {
        ...
        // save original default locale value for later checks (should stay the
        // same)
        ...
        assertTrue("The one seedlist should be named defaultseeds", "defaultseeds".equals(s0.getName()));
        ...
        assertTrue("At 1. check Locale default value has change from " + origDefault.getLanguage() + " to "
                + Locale.getDefault().getLanguage(), Locale.getDefault().getLanguage() == origDefault.getLanguage());
        ...
        // With java 5:
        // DK sequence is "abe liste", "Abe liste", "Anden liste",
        // "defaultseeds", "ny liste", "Plidderpladder"
        ...
        CollectionAsserts.assertListEquals("List of seeds should have expected content, in order", allSeeds, s4, s5,
                s3, defaultSeeds, s2, s1, s9, s8, s7, s6);

        List<String> names = extractNamesFromItems(s4, s5, s3, defaultSeeds, s2, s1, s9, s8, s7, s6);
        ...
        // check default locale value
        assertTrue("At 2. check Locale default value has change from " + origDefault.getLanguage() + " to "
                + Locale.getDefault().getLanguage(), Locale.getDefault().getLanguage() == origDefault.getLanguage());
        ...
        // check default locale value
        assertTrue("At 3. check Locale default value has change from " + origDefault.getLanguage() + " to "
                + Locale.getDefault().getLanguage(), Locale.getDefault().getLanguage() == origDefault.getLanguage());
        ...
        assertTrue("defaultConfig must have name 'defaultconfig', not " + defaultConfig.getName(), defaultConfig
                .getName().equals("defaultconfig"));
        ...
        // check default locale value
        assertTrue("At 4. check Locale default value has change from " + origDefault.getLanguage() + " to "
                + Locale.getDefault().getLanguage(), Locale.getDefault().getLanguage() == origDefault.getLanguage());
        ...
        // check default locale value
        assertTrue("At 5. check Locale default value has change from " + origDefault.getLanguage() + " to "
                + Locale.getDefault().getLanguage(), Locale.getDefault().getLanguage() == origDefault.getLanguage());
        ...
        // check default locale value
        assertTrue("At 6. check Locale default value has change from " + origDefault.getLanguage() + " to "
                + Locale.getDefault().getLanguage(), Locale.getDefault().getLanguage() == origDefault.getLanguage());
        ...
        // check default locale value
        assertTrue("At 7. check Locale default value has change from " + origDefault.getLanguage() + " to "
                + Locale.getDefault().getLanguage(), Locale.getDefault().getLanguage() == origDefault.getLanguage());
    }
