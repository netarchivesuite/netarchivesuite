...
public class HTTPControllerServerTester {
    ...
    public void setUp() throws Exception {
        ...
        startRequest = new TestRequest(new URI("http://" + hostname + "/startRecordingURIs?returnURL=" + returnUrl));
        stopRequest = new TestRequest(new URI("http://" + hostname + "/stopRecordingURIs?returnURL=" + returnUrl));
        ArrayList<String> values = new ArrayList<String>();
        values.add("http://foo.bar");
        clearRequest = new TestRequest(new URI("http://" + hostname + "/clearRecordedURIs"));
        clearRequest.postParameters.put("returnURL", values);
        getRecordedURIsRequest = new TestRequest(new URI("http://" + hostname + "/getRecordedURIs"));
        changeIndexRequest = new TestRequest(new URI("http://" + hostname
                + "/changeIndex?jobID=1&label=myTestLabel&returnURL=" + returnUrl));
        changeIndexRequest2 = new TestRequest(new URI("http://" + hostname
                + "/changeIndex?jobID=1&label=myTestLabel&returnURL=" + returnUrl));
        values = new ArrayList<String>();
        values.add("8");
        changeIndexRequest2.postParameters.put("jobID", values);
        changeIndexRequest2.postParameters.put("label", Arrays.asList("myTestLabel"));
        wrongRequests = new TestRequest[] {
                new TestRequest(new URI("http://" + hostname + "/startRecordingURIs?fnidder=fnadder")),
                new TestRequest(new URI("http://" + hostname + "/stopRecordingURIs?fnidder=fnadder")),
                new TestRequest(new URI("http://" + hostname + "/clearRecordedURIs?fnidder=fnadder")),
                new TestRequest(new URI("http://" + hostname + "/changeIndex?returnURL=" + returnUrl)),
                new TestRequest(new URI("http://" + hostname + "/changeIndex?jobID=" + 1)),
    ...
    public void testSetURIResolver() throws Exception {
        ...
        assertEquals("Should NOT have called lookup method on uri resolver", 0, ur.lookupCount);
        assertEquals("Should have called start on controller", 1, c.startCounter);
        cs.lookup(new TestRequest(new URI("http://foo.bar")), response);
        assertEquals("Should have called lookup method on uri resolver", 1, ur.lookupCount);
        TestURIResolver ur2 = new TestURIResolver();
        cs.setURIResolver(ur2);
        cs.lookup(startRequest, response);
        assertEquals("Should NOT have called lookup method on uri resolver", 0, ur2.lookupCount);
        assertEquals("Should have called start on controller", 2, c.startCounter);
        cs.lookup(new TestRequest(new URI("http://foo.bar")), response);
        assertEquals("Should NOT have called lookup method on uri resolver", 1, ur.lookupCount);
        assertEquals("Should have called lookup method on uri resolver 2", 1, ur2.lookupCount);
    }
    ...
    public void testLookup() throws Exception {
        ...
        assertEquals("Should NOT have called lookup method on uri resolver", 0, ur.lookupCount);
        assertEquals("Should have called start on controller", 1, c.startCounter);
        assertEquals("Should have called nothing else on controller", 1, c.totalCounter);
        assertEquals("Response should be redirect", 303, result);
        assertEquals("Response needs redirect status code", 303, response.status);
        assertEquals("Nothing should be written to body", "", response.os.toString());
        assertEquals("Response must now include one extra header", 1, response.headerFields.size());
        assertEquals("Response must now include location header", "Location", response.headerFields.get(0).getKey());
        assertEquals("Response must now include location header to foo.bar", "http://foo.bar", response.headerFields
                .get(0).getValue());
        ...
        assertEquals("Should NOT have called lookup method on uri resolver", 0, ur.lookupCount);
        assertEquals("Should have called start on controller", 1, c.stopCounter);
        assertEquals("Should have called nothing else on controller", 2, c.totalCounter);
        assertEquals("Response should be redirect", 303, result);
        assertEquals("Response should be redirect", 303, result);
        assertEquals("Response needs redirect status code", 303, response.status);
        assertEquals("Nothing should be written to body", "", response.os.toString());
        ...
        assertEquals("Should NOT have called lookup method on uri resolver", 0, ur.lookupCount);
        assertEquals("Should have called start on controller", 1, c.clearCounter);
        assertEquals("Should have called nothing else on controller", 3, c.totalCounter);
        assertEquals("Response should be redirect", 303, result);
        assertEquals("Response should be redirect", 303, result);
        assertEquals("Response needs redirect status code", 303, response.status);
        assertEquals("Nothing should be written to body", "", response.os.toString());
        ...
        assertEquals("Should NOT have called lookup method on uri resolver", 0, ur.lookupCount);
        assertEquals("Should have called start on controller", 1, c.getRecordedURICounter);
        assertEquals("Should have called nothing else on controller", 4, c.totalCounter);
        assertEquals("Response should be OK", 200, result);
        assertEquals("Response needs OK status code", 200, response.status);
        assertEquals("URL list should be written to body", "http://bar.foo\n", response.os.toString());
        assertEquals("Response must now include one extra header", 1, response.headerFields.size());
        assertEquals("Response must now include content type header", "Content-Type", response.headerFields.get(0)
                .getKey());
        assertEquals("Response must now include content type header text/plain", "text/plain; charset=UTF-8",
                response.headerFields.get(0).getValue());

        // Check changeIndex command with one argument
        response.reset();
        result = cs.lookup(changeIndexRequest, response);
        assertEquals("Should NOT have called lookup method on uri resolver", 0, ur.lookupCount);
        assertEquals("Should have called changeIndex on controller", 1, c.changeIndexCounter);
        assertEquals("Should have called nothing else on controller", 5, c.totalCounter);
        assertEquals("Response should be redirect", 303, result);
        assertEquals("Response needs redirect status code", 303, response.status);
        assertEquals("Nothing should be written to body", "", response.os.toString());
        assertEquals("Response must now include one extra header", 1, response.headerFields.size());
        ...
        assertEquals("Response must now include location header to foo.bar", "http://foo.bar", response.headerFields
                .get(0).getValue());
        assertEquals("Should have received one job id", 1, c.changeIndexJobListArgument.size());
        assertEquals("Should have received job id '1'", Long.valueOf(1L), c.changeIndexJobListArgument.toArray()[0]);
        ...
        assertEquals("Should NOT have called lookup method on uri resolver", 0, ur.lookupCount);
        assertEquals("Should have called changeIndex on controller", 2, c.changeIndexCounter);
        assertEquals("Should have called nothing else on controller", 6, c.totalCounter);
        assertEquals("Response should be redirect", 303, result);
        assertEquals("Response needs redirect status code", 303, response.status);
        assertEquals("Nothing should be written to body", "", response.os.toString());
        assertEquals("Response must now include one extra header", 1, response.headerFields.size());
        ...
        assertEquals("Response must now include location header to foo.bar", "http://foo.bar", response.headerFields
                .get(0).getValue());
        assertEquals("Should have received two job ids", 2, c.changeIndexJobListArgument.size());
        ...
        for (TestRequest wrongRequest : wrongRequests) {
            ...
            assertEquals("Should NOT have called lookup method on uri resolver", 0, ur.lookupCount);
            assertEquals("Should NOT have called anything on controller", 6, c.totalCounter);
        }

        // Check non-command url
        result = cs.lookup(new TestRequest(new URI("http://non.command.url")), response);
        assertEquals("Should have called lookup method on uri resolver", 1, ur.lookupCount);
        assertEquals("Should have called nothing on controller", 6, c.totalCounter);
        assertEquals("Result should be set by uri resolver", 42, result);
        assertEquals("Response should have been received by uri resolver", response, ur.lookupResponseArgument);
        assertEquals("URI should have been received by uri resolver", new URI("http://non.command.url"),
                ur.lookupRequestArgument.getURI());

        // It's not up to the controller to handle null in lookup
        result = cs.lookup(new TestRequest(new URI("http://non.command.url")), null);
        assertEquals("Should have called lookup method on uri resolver", 2, ur.lookupCount);
        assertEquals("Should have called nothing on controller", 6, c.totalCounter);
        assertEquals("Result should be set by uri resolver", 42, result);
        assertEquals("Null should have been received by uri resolver", null, ur.lookupResponseArgument);
        assertEquals("URI should have been received by uri resolver", new URI("http://non.command.url"),
                ur.lookupRequestArgument.getURI());

        // It's not up to the controller to handle null in lookup
        result = cs.lookup(null, response);
        assertEquals("Should have called lookup method on uri resolver", 3, ur.lookupCount);
        assertEquals("Should have called nothing on controller", 6, c.totalCounter);
        assertEquals("Result should be set by uri resolver", 42, result);
        assertEquals("Response should have been received by uri resolver", response, ur.lookupResponseArgument);
        assertEquals("null should have been received by uri resolver", null, ur.lookupRequestArgument);
    ...
    public static class TestResponse implements Response {
        ...
        int status = 0;
        String reason = "";
        List<KeyValuePair<String, String>> headerFields = new ArrayList<KeyValuePair<String, String>>();
                ...
                for (String parameter : queryParts) {
                    ...
                    if (parameterParts.length == 2) {
                        ...
                        try {
                            ...
                            throw new ArgumentNotValid("This should never happen: " + "UTF-8 is an"
                                    + " unsupported charset.");
                        }
                    }
                }
