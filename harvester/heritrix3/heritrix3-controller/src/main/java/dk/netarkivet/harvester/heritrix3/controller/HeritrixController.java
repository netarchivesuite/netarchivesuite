/*
 * #%L
 * NetarchiveSuite - heritrix3 controller
 * %%
...
public class HeritrixController extends AbstractRestHeritrixController {
     ...
     *
     * @throws IOFailure If Heritrix3 dies before initialisation, or we encounter any problems during the
     * initialisation.
...
//            e.printStackTrace();
    ...
    public void stopHeritrix() {
        ...
        try {
            ...
            if (processBuilder.start().waitFor() == 0) { // Yes, ask heritrix3 to shutdown, ignoring any jobs named
                                                         // jobName
    ...
    private void fetchCrawlServiceJobAttributes(CrawlProgressMessage cpm, JobResult jobResult) {
         ...
         * congestion max-depth avg-depth 2015-04-29T12:42:54Z 774 573 185 0.9(2.31) 49(41) 16 2 61249 270848 1 456 114
         */
        /*
         * jStatus.setProgressStatistics(newProgressStats); if (progressStatisticsLegend == null) {
        ...
        case "PAUSED":
            ...
            break;
        case "EMPTY":
            cpm.setStatus(CrawlStatus.CRAWLER_EMPTY);
            break;
        default:
    ...
    public FullFrontierReport getFullFrontierReport() {
        ...
        try {
            postEntity = new StringEntity("engine=beanshell&script="
                    + dk.netarkivet.harvester.heritrix3.Constants.FRONTIER_REPORT_GROOVY_SCRIPT);
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
