...
public interface HeritrixController {
    ...
    /**
     * Tell Heritrix to stop crawling. Heritrix may take a while to actually stop, so you cannot assume that crawling is
     * stopped when this method returns.
     */
    void beginCrawlStop();

    /**
     * Request that crawling stops. This makes a call to beginCrawlStop(), unless the crawler is already shutting down.
    ...
    /**
     * Query whether Heritrix is in a state where it can finish crawling. Returns true if no uris remain to be
     * harvested, or it has met either the maxbytes limit, the document limit, or the time-limit for the current
     * harvest.
     *
     ...
     *
     * @return True if the CrawlEnded event has happened in Heritrix, indicating that all crawls have stopped.
     */
    ...
    /**
     * Get the number of URIs currently on the queue to be processed. This number may not be exact and should only be
     * used in informal texts.
     *
    ...
    /**
     * Get a human-readable set of statistics on the progress of the crawl. The statistics is discovered uris, queued
     * uris, downloaded uris, doc/s(avg), KB/s(avg), dl-failures, busy-thread, mem-use-KB, heap-size-KB, congestion,
     * max-depth, avg-depth. If no statistics are available, the string "No statistics available" is returned. Note:
     * this method may disappear in the future.
     *
