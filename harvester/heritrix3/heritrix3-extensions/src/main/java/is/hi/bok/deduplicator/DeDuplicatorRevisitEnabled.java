...
public class DeDuplicatorRevisitEnabled extends Processor implements InitializingBean {
    ...
    protected DedupStatistics stats = null;
    protected HashMap<String, DedupStatistics> perHostStats = null;
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        stats = new DedupStatistics();
        if (statsPerHost) {
            perHostStats = new HashMap<String, DedupStatistics>();
        }
    }
	...
	protected ProcessResult innerProcessResult(CrawlURI curi) throws InterruptedException {
        ...
        DedupStatistics currHostStats = null;
        if(statsPerHost){
            synchronized (perHostStats) {
                ...
                if(currHostStats==null){
                    currHostStats = new DedupStatistics();
 ...
 /*           
     ...
     */
    protected Document lookupByURL(CrawlURI curi, DedupStatistics currHostStats) {
     ...
     * @return The result of the lookup (a Lucene document). If a duplicate is not found null is returned.
     */
    protected Document lookupByDigest(CrawlURI curi, DedupStatistics currHostStats) {
        Document duplicate = null;
    ...
    public String report() {
        ...
        if(statsPerHost){
            ...
            synchronized (perHostStats) {
                ...
                while(it.hasNext()){
                    String key = it.next();
                    DedupStatistics curr = perHostStats.get(key);
                    ret.append("  " +key);
                    ret.append(" ");
	...
	protected void doAnalysis(CrawlURI curi, DedupStatistics currHostStats,
            boolean isDuplicate) {
		try{
    		Query query = queryField(DigestIndexer.FIELD_URL, curi.toString());
            AllDocsCollector collectAllCollector = new AllDocsCollector();
	...
	protected void doTimestampAnalysis(CrawlURI curi, Document urlHit, 
			DedupStatistics currHostStats, boolean isDuplicate){
