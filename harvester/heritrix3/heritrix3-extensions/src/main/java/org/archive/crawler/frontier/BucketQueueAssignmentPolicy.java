...
public class BucketQueueAssignmentPolicy extends QueueAssignmentPolicy {
    ...
    private static Logger LOGGER = 
            Logger.getLogger(BucketQueueAssignmentPolicy.class.getName());
    ...
    public String getClassKey(final CrawlURI curi) {
        ...
        if(host == null) {
        	...
        	queue = "NO-IP-".concat(Long.toString(Math.abs((long) host
                    .getHostName().hashCode()) & DEFAULT_NOIP_BITMASK));
        } else {
        	queue = Long.toString(Math.abs((long) host.getIP().hashCode())
                    % DEFAULT_QUEUES_HOSTS_MODULO);
        }
        LOGGER.info("Found classkey: " + queue);
        return queue;
    }
