...
implements HasKeyedProperties, 
    ...
    throws InterruptedException {
        ...
        if (shouldProcess(uri)) {
        	logger.info("curi " + uri + " should be processed by bean" + this.getBeanName());
            uriCount.incrementAndGet();
            ProcessResult result = innerProcessResult(uri);
            logger.info("curi " + uri + " is allowed to proceed to the next bean: " + (result==ProcessResult.PROCEED? "YES":"NO"));
            return result;
        } else {
        	logger.info("curi " + uri + " is allowed to proceed to the next bean");
            return ProcessResult.PROCEED;
