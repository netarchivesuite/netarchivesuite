    ...
    public void run() {
        ...
        try {
            while ( true ) {
                ...
                synchronized(this) {
                	// Tentative fix for https://sbforge.org/jira/browse/NAS-2560
                	try {
                		// Report the URI (currentCuri) being processed as having finished processing.
                		controller.getFrontier().finished(currentCuri);
                	} catch (NullPointerException e) {
                		// Let's try ignoring this NullPointerException and keep on processing
                		logger.log(Level.SEVERE, "NPE caught during call to .getFrontier().finished "
                				+ " with argument currentCuri = '" + currentCuri + '"', e);
                	}
