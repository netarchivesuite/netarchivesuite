...
/**
 * Wraps information for an Heritrix file that should be 
...
class MetadataFile implements Comparable<MetadataFile> {
	...
	/**
	 * A string format that is used to build metadata URLs. 
    ...
    /**
     * The pattern controlling which files in the logs subdirectory of the 
     * crawl directory root should be stored in the metadata ARC as log files.
     */
    ...
    public int compareTo(MetadataFile other) {
        Integer thisOrdinal = new Integer(this.type.ordinal());
        Integer otherOrdinal = new Integer(other.type.ordinal());
