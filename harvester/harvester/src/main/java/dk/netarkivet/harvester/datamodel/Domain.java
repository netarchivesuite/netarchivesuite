...
public class Domain implements Named {
    ...
    private List<ExtendedFieldValue> extendedFieldValues;
    ...
    protected Domain(String theDomainName) {
        ...
        extendedFieldValues = new ArrayList<ExtendedFieldValue>();
    }
    ...
    public ExtendedFieldValue getExtendedFieldValue(Long aExtendedFieldId) {
        for (int i = 0; i < extendedFieldValues.size(); i++) {
            if (extendedFieldValues.get(i).getExtendedFieldID()
                    .equals(aExtendedFieldId)) {
     ...
     * or how many objects a harvest using a given configuration will retrieve,
     * we will prioritise the most recently harvest, where we have a full
     * harvest.
