...
public class FullHarvest extends HarvestDefinition {
    ...
    /**
     * The maximum time in seconds to run for each job generated by this
     * definition.
     */
     ...
     * of the supplied DomainConfiguration. Should only be used by the
     * HarvestFactory class.
     *
     * @param harvestDefName
     *            the name of the harvest definition
     * @param comments
     *            comments
     * @param previousHarvestDefinitionOid
     *            This harvestDefinition is used to create this Fullharvest
     *            definition.
     * @param maxCountObjects
     *            Limit for how many objects can be fetched per domain
     * @param maxBytes
     *            Limit for how many bytes can be fetched per domain
     * @param maxJobRunningTime
     *            Limit on how much time can be spent on each job. 0 means no
     *            limit
     * @param isIndexReady
     ...
     * Domains are filtered out if, on the previous harvest, they: 1) were
     * completed 2) reached their maxBytes limit (and the maxBytes limit has not
     * changed since time of harvest) 3) reached their maxObjects limit (and the
     * maxObjects limit has not changed since time of harvest) 4) died uncleanly
     * (e.g. due to a manual shutdown of heritrix) on their last harvest.
     *
    ...
    /**
     * Get the configuration used in a previous harvest. If the configuration in
     * the harvestinfo cannot be found (deleted), uses the default
     * configuration.
     *
     * @param harvestInfo
     *            A harvest info object from a previous harvest.
     * @param dao
     *            The dao to read configurations from.
     *
    ...
    /**
     * Set the limit for how many seconds each crawljob in this fullharvest will
    ...
    /**
     * Is index ready. Used to check, whether or a FullHarvest is ready for
     * scheduling. The scheduling requires, that the deduplication index used by
     * the jobs in the FullHarvest, has allready been prepared by the
     * IndexServer.
     * 
