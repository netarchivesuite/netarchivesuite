...
/**
 * This interface encapsulates the direct access to Heritrix, allowing for
 * accessing in various ways (direct class access or JMX). Heritrix is expected
 * to perform one crawl for each instance of an implementing class.
 *
 */
public interface HeritrixController {

    /**
     * Initialize a new CrawlController for executing a Heritrix crawl. This
     * does not start the crawl.
     */
    void initialize();

    /**
     * Request that Heritrix start crawling. When this method returns, either
     * Heritrix has failed in the early stages, or the crawljob has been
     * successfully created. Actual crawling will commence at some point
     * hereafter.
     * 
     * @throws IOFailure
    ...
    /**
     * Request that crawling stops. This makes a call to beginCrawlStop(),
     * unless the crawler is already shutting down. In that case it does
     * nothing.
     *
     * @param reason
     ...
     * Returns true if no uris remain to be harvested, or it has met either the
     * maxbytes limit, the document limit, or the time-limit for the current
     * harvest.
     *
     ...
     *
     * @return True if the CrawlEnded event has happened in Heritrix, indicating
     *         that all crawls have stopped.
     */
    ...
    /**
     * Get an estimate of the rate, in kb, at which documents are currently
     * being processed by the crawler.
     * 
    ...
    /**
     * Get a human-readable set of statistics on the progress of the crawl. The
     * statistics is discovered uris, queued uris, downloaded uris, doc/s(avg),
     * KB/s(avg), dl-failures, busy-thread, mem-use-KB, heap-size-KB,
     * congestion, max-depth, avg-depth. If no statistics are available, the
     * string "No statistics available" is returned. Note: this method may
    ...
    /**
     * Get harvest information. An example of this can be an URL pointing to the
     * GUI of a running Heritrix process.
     * 
