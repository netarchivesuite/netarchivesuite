...
public class HarvesterSettings {
     ...
     * The newly created configurations use this. This template must exist
     ...
     * during job-creation process. This defines how great a possible factor we
     * will permit a harvest to be larger then the expectation, when basing the
    ...
    /**
     * <b>settings.harvester.scheduler.jobGen.config.
     * expectedAverageBytesPerObject</b>: <br>
     ...
     * The initial guess of the domain size (number of objects) of an unknown
     * domain.
     */
    public static String MAX_DOMAIN_SIZE = "settings.harvester.scheduler.jobGen.config.maxDomainSize";

    /**
     * <b>settings.harvester.scheduler.jobGen.config.maxRelativeSizeDifference</
     * b>: <br>
    ...
    /**
     * <b>settings.harvester.scheduler.jobGen.config.minAbsoluteSizeDifference</
     * b>: <br>
     ...
     * The limit on how many seconds Heritrix should continue on each job. O
     * means no limit.
     */
     ...
     * How many domain configurations we will process in one go before making
     * jobs out of them. This amount of domains will be stored in memory at the
     * same time. To avoid job splitting, set this value as Long.MAX_VALUE.
     */
    public static String JOBGEN_DOMAIN_CONFIG_SUBSET_SIZE = "settings.harvester.scheduler.jobGen.domainConfigSubsetSize";

    /**
     * <b>settings.harvester.scheduler.jobGen.config.fixedDomainCountFocused</b>
     * : <br>
     * If the job generator is {@link FixedDomainConfigurationCountJobGenerator}
     * , then this parameter represents the maximum number of domain
     * configurations in a partial harvest job.
     */
    ...
    /**
     * <b>settings.harvester.scheduler.jobGen.config.fixedDomainCountSnapshot</b
     * >: <br>
     * If the job generator is {@link FixedDomainConfigurationCountJobGenerator}
     * , then this parameter represents the maximum number of domain
     * configurations in a full harvest job.
     */
    ...
    /**
     * <b>settings.harvester.scheduler.jobGen.config.
     * excludeDomainsWithZeroBudget</b>: <br>
     * If the job generator is {@link FixedDomainConfigurationCountJobGenerator}
     * , then this parameter toggles whether or not domain configurations with a
     * budget of zero (byte or objects) should be excluded from jobs. The
    ...
    /**
     * <b>settings.harvester.scheduler.jobGen.config.postponeUnregisteredChannel
     * </b>: <br>
     * If this property is true, then the job generator will postpone job
     * generation for harvest definitions that are mapped to a harvest channel
     * not registered to at least one harvester. The default value is 'true'.
     */
     ...
     * The fully qualified class name of the chosen job generator
     * implementation, currently either {@link DefaultJobGenerator} or
     * {@link FixedDomainConfigurationCountJobGenerator}. The default is
     ...
     * parameter is set to "true", then the object limit is used instead as the
     * base criterion.
     */
    public static String SPLIT_BY_OBJECTLIMIT = "settings.harvester.scheduler.jobGen.config.splitByObjectLimit";

    /**
     * <b>settings.harvester.scheduler.jobGen.objectLimitIsSetByQuotaEnforcer</b
     * >: <br>
     ...
     *
     * Default value is true, as legacy implementation was to use only the
     * QuotaEnforcer.
     */
     ...
     * Time before a STARTED job times out and change status to FAILED. In
     * seconds.
     */
     ...
     * Each job gets a subdir of this dir. Job data is written and Heritrix
     * writes to that subdir.
     */
     ...
     * The minimum amount of free bytes in the serverDir required before
     * accepting any harvest-jobs.
     */
     ...
     * Harvest channel to take jobs from. This is the default channel assigned
     * to the harvest controller.
     * 
     ...
     * The timeout setting for aborting a crawl based on crawler-inactivity. If
     * the crawler is inactive for this amount of seconds the crawl will be
     * aborted. The inactivity is measured on the
     ...
     * crawl when no bytes are being received from web servers.
     */
     ...
     * data for a running job. Default value is 5 minutes.
     */
     ...
     * Maximum number of most recent history records displayed on the running
     * job details page.
     */
     ...
     * Time interval in seconds to wait during a crawl loop in the harvest
     * controller. Default value is 20 seconds.
     */
     ...
     * {@link HarvesterReadyMessage} to the {@link JobDispatcher}.
     * <p>
     * 
     * Lower values will make the JobDispatcher detect ready harvester faster,
     * but will make it more likely that the harvester may send two ready
     * messages before a job is received, causing the JobDispatcher to dispatch
     * two jobs.
     * 
     ...
     * the ready message to the harvester. Default value is 1000 millisecond.
     */
    ...
    /**
     * <b>settings.harvester.harvesting.frontier.filter.class</b> Defines a
     * filter to apply to the full frontier report. the default class:
     * {@link TopTotalEnqueuesFilter}
     */
    public static String FRONTIER_REPORT_FILTER_CLASS = "settings.harvester.harvesting.frontier.filter.class";

    /**
     * <b>settings.harvester.harvesting.frontier.filter.args</b> Defines a
     * frontier report filter's arguments. Arguments should be separated by
     * semicolons.
     */
     ...
     * manually. Default value is true.
     * 
    ...
    /**
     * <b>settings.harvester.harvesting.heritrix.waitForReportGenerationTimeout<
     * /b>:<br>
     ...
     * Port used to access the Heritrix web user interface. This port must not
     * be used by anything else on the machine. Note that apart from pausing a
     * job, modifications done directly on Heritrix may cause unexpected
     * breakage.
     */
     ...
     * The port that Heritrix uses to expose its JMX interface. This port must
     * not be used by anything else on the machine, but does not need to be
     * accessible from other machines unless you want to be able to use jconsole
     * to access Heritrix directly. Note that apart from pausing a job,
     ...
     * The username used to connect to Heritrix JMX interface The username must
     ...
     * The password used to connect to Heritrix JMX interface The password must
     * correspond to the value stored in the jmxremote.password file (name
     * defined in setting settings.common.jmx.passwordFile).
     */
     ...
     * The heap size to use for the Heritrix sub-process. This should probably
     * be fairly large. It can be specified in the same way as for the -Xmx
     * argument to Java, e.g. 512M, 2G etc.
     */
     ...
     * Additional JVM options for the Heritrix sub-process. By default there is
     * no additional JVM option.
     */
     ...
     * Should we disregard seedURL-information and thus assign the harvested
     * bytes to the domain of the harvested URL instead of the seed url domain?
     ...
     * This setting tells the system whether or not to use deduplication. This
    ...
    /**
     * <b>settings.harvester.harvesting.metadata.generateArchiveFilesReport</b>
     * This setting is a boolean flag that enables/disables the generation of an
     * ARC/WARC files report. Default value is 'true'.
     *
     ...
     * {@link #METADATA_GENERATE_ARCHIVE_FILES_REPORT} is set to true, sets the
     * name of the generated report file. Default value is
     ...
     * <b>settings.harvester.harvesting.metadata.archiveFilesReportName</b> If
     * {@link #METADATA_GENERATE_ARCHIVE_FILES_REPORT} is set to true, sets the
     * header of the generated report file. This setting should generally be
     * left to its default value, which is '[ARCHIVEFILE] [Opened] [Closed]
     * [Size]'.
     *
     * @see HarvestDocumentation#documentHarvest(dk.netarkivet.harvester.harvesting.IngestableFiles)
     */
    ...
    /**
     * <b>settings.harvester.aliases.timeout</b> The amount of time in seconds
     * before an alias times out, and needs to be re-evaluated. The default
     * value is one year, i.e 31536000 seconds.
     */
    public static String ALIAS_TIMEOUT = "settings.harvester.aliases.timeout";

    /**
     * <b>settings.harvester.harvesting.
     * continuationFromHeritrixRecoverlogEnabled</b>:</br> Setting for whether
     * or not a restarted job should try fetching the recoverlog of the previous
     * failed job, and ask Heritrix to continue from this log. The default is
     * false.
     */
    ...
    /**
     * <b>settings.harvester.harvesting.metadata.metadataFormat</b> The
     * dataformat used by Netarchivesuite to write the metadata associated with
     * a given harvest job. default: arc (alternative: warc)
     */
    public static String METADATA_FORMAT = "settings.harvester.harvesting.metadata.metadataFormat";

    /**
     * <b>settings.harvester.harvesting.heritrix.archiveFormat</b> The
     * dataformat used by heritrix to write the harvested data. default: warc
     * (alternative: arc)
     */
    public static String HERITRIX_ARCHIVE_FORMAT = "settings.harvester.harvesting.heritrix.archiveFormat";
    /**
     * <b>settings.harvester.harvesting.heritrix.archiveNaming.class</b> The
     * class implementing the chosen way of naming your archive-files default:
     * LegacyNamingConvention. This class decides what to put into the Heritrix
     * "prefix" property of the org.archive.crawler.writer.ARCWriterProcessor
     * and/or org.archive.crawler.writer.WARCWriterProcessor.
     */
     ...
     * Represents the 'skip-identical-digests' setting in the Heritrix
     * WARCWriterProcessor. The default is false.
     */
     ...
     * Represents the 'write-requests' setting in the Heritrix
     * WARCWriterProcessor. The default is true
     */
     ...
     * Represents the 'write-metadata' setting in the Heritrix
     * WARCWriterProcessor. The default is false.
     */
    public static String HERITRIX_WARC_WRITE_METADATA = "settings.harvester.harvesting.heritrix.warc.writeMetadata";
    /**
     * <b>settings.harvester.harvesting.heritrix.warc.
     * writeRevisitForIdenticalDigests</b> Represents the
     * 'write-revisit-for-identical-digests' setting in the Heritrix
     * WARCWriterProcessor. The default is false.
     */
    ...
    /**
     * <b>settings.harvester.harvesting.heritrix.warc.writeRevisitForNotModified
     * </b> Represents the 'write-revisit-for-not-modified' setting in the
     ...
     * Setting for the max number of threads the deduplication indexer shall
     * use.
     */
     ...
     * Setting for the time in milliseconds between each check of the state of
     * sub-indexing. Default: 30 seconds (30000 milliseconds).
     */
     ...
     * Setting for how many segments we will accept in our lucene indices. The
     * default is 15.
     */
     ...
     * Setting for the satisfactory threshold of the indexing result as a
     * percentage. The default is 70 percent
     */
     ...
     * Setting for whether or not data not found in the default bitarchive
     * replica shall be looked for in other bitarchive replicas. The default is
     * false.
     */
     ...
     * The main directory for the ViewerProxy, used for storing the Lucene index
     * for the jobs being viewed. This directory can be used by multiple
     ...
     * If we fail to lookup an URI, we will try changing the protocol to ftp, if
     * this setting is set to true. The default is false.
     *
     */
    public static String TRY_LOOKUP_URI_AS_FTP = "settings.harvester.viewerproxy.tryLookupUriAsFtp";

    /**
     * <b>settings.viewerproxy.maxSizeInBrowser</b> The size (in bytes) of the
     * largest object to be returned for viewing in the browser window. Larger
     * objects will be returned with the appropriate http header for saving them
     * to a file.
     */
    public static String MAXIMUM_OBJECT_IN_BROWSER = "settings.harvester.viewerproxy.maxSizeInBrowser";

    /**
     * The maximum length (in lines) of crawllog to be displayed in a browser
     * window.
     */
