...
public class DailyFrequency extends Frequency {
     ...
     *
     * @param numUnits
     *            Number of days from event to event.
    ...
    /**
     * Create a new daily frequency that happens every numUnits days, on the
     * given hour and minute.
     *
     * @param numUnits
     *            Number of days from event to event.
     * @param hour
     *            The hour on which the event should happen.
     * @param minute
     *            The minute of hour on which the event should happen.
     * @throws ArgumentNotValid
     *             if numUnits if 0 or negative or hours is <0 or >23 or minutes
     *             is <0 or >59
     */
     ...
     *
     * The time of the next event is guaranteed to be later that lastEvent. For
     * certain frequencies (e.g. once a day, any time of day), the time of the
     * next event is derived from lastEvent, for others (e.g. once a day at
     * 13:00) the time of the next event is the first matching time after
     * lastEvent.
     *
     * @param lastEvent
    ...
    public boolean equals(Object o) {
        if (this == o)
            return true;
        if (!(o instanceof DailyFrequency))
            return false;
        if (!super.equals(o))
            return false;
        ...
        if (hour != dailyFrequency.hour)
            return false;
        if (minute != dailyFrequency.minute)
            return false;
    ...
    /**
     * Return the exact minute event should happen on, or null if this is an
     * anyTime event or doesn't define what minute it should happen on.
     *
     * @return the exact minute event should happen on, or null if this is an
     *         anyTime event or doesn't define what minute it should happen on
     */
    ...
    /**
     * Return the exact hour event should happen on, or null if this is an
     * anyTime event or doesn't define what hour it should happen on.
     *
     * @return the exact hour event should happen on, or null if this is an
     *         anyTime event or doesn't define what hour it should happen on
     */
     ...
     *
     * @return the exact day of week event should happen on, or null if this is
     *         an anyTime event or doesn't define what day of week it should
     *         happen on
     */
     ...
     *
     * @return the exact day of month event should happen on, or null if this is
     *         an anyTime event or doesn't define what day of month it should
     *         happen on
     */
    ...
    /**
     * Return an integer that can be used to identify the kind of frequency. No
     * two subclasses should use the same integer
     *
