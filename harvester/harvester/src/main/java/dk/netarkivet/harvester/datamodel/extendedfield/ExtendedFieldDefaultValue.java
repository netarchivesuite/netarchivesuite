...
public class ExtendedFieldDefaultValue {
    ...
    private boolean validate() {
        ...
        case ExtendedFieldDataTypes.NUMBER:
            if (value == null || value.length() == 0) { // no value no format
                                                        // check
                isValid = true;
                break;
            }
    ...
    public String getDBValue() {
        // prevent that any null value fills content column of
        // extendedFieldValue
        if (value == null) {
            value = "";
        }

        // only if datatype is Timestamp, JSCalendar or Number. Otherwise
        // DB-Value = Value
