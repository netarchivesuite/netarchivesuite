...
/**
 * A Data Access Object for harvest definitions. This object is a singleton to
 * ensure thread-safety. It handles the transformation from harvest definitions
...
public abstract class HarvestDefinitionDAO implements DAO, Iterable<HarvestDefinition> {
     ...
     * @throws IOFailure
     *             if unable to create the singleton.
     */
     ...
     * @throws IOFailure
     *             if the File does not exist, does not have the correct ID, or
    ...
    /**
     * Update an existing harvest definition with new info in persistent
     * storage.
     *
     * @param harvestDefinition
     *            An updated harvest definition object to be persisted.
     */
    public abstract void update(HarvestDefinition harvestDefinition);

    /**
     * Activates or deactivates a partial harvest definition, depending on its
     * activation status.
     * 
     * @param harvestDefinition
    ...
    /**
     * Get an iterator of all harvest definitions. Implements the Iterable
     * interface.
     *
     ...
     *
     * @param now
     * @return IDs of the harvest definitions that are currently ready to be
     *         scheduled. Some of these might already be in the process of being
     *         scheduled.
     */
    public abstract Iterable<Long> getReadyHarvestDefinitions(Date now);

    /**
     * Get the harvest definition that has the given name, or null, if no
     * harvestdefinition exist with this name.
     * 
     * @param name
    ...
    /**
     * Returns a list with information on the runs of a particular harvest. The
     * list is ordered by descending run number.
     *
     * @param harvestID
     ...
     * @return Domain,configuration pairs for that HD. Returns an empty list for
     *         unknown harvest definitions.
     ...
     *            If true only active harvest definitions are returned.
     ...
     * 
     * @param harvestId
     *            Id for a specific PartialHarvest
     * @param key
     *            a SparseDomainConfiguration uniquely identifying the
     *            domainconfig.
     */
     ...
     * for the next harvestrun. If no selective harvest matching the given id is
     * found in the storage, the method should silently return.
     * 
     * @param harvestId
     *            A given PartialHarvest id (i.e. Selective Harvest).
     * @param nextdate
     ...
     * 
     * @param hdd
     *            a given PartialHarvest
     * @param sparseDomainConfiguration
