...
public enum JobPriority {
    ...
    /**
     * Builds a {@link JobPriority} from a string.
     * @param pString the string representing the enum name, case-insensitive.
     * @return the corresponding {@link JobPriority}
     * @throws UnknownID if the string does not match any enum value.
     */
    public static JobPriority fromString(String pString) {
        for (JobPriority p : values()) {
            if (p.name().equalsIgnoreCase(pString)) {
                return p;
            }
        }
        throw new UnknownID(pString + " is not a valid job priority");
    }
}
