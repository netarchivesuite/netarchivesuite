...
/**
 * An abstract superclass for URIResolvers that handle commands given to the
 * server host (http://<<serverhost>>/<<command>>/<<param>>=<<value>>*).
 */
...
public abstract class CommandResolver implements URIResolverHandler, URIResolver {
     ...
     *
     * @param ur
     *            The URIResolver that handles URIs that are not handled by this
     *            resolver.
     */
     ...
     *
     * @param anUr
     *            The URI resolver to handle unhandled URIs.
     ...
     * http://<<localhostname>>/<<command>>?<<param>>=<<value>>* where command
     * and param are defined in the subclass.
     *
     ...
     * @param response
     *            HTTP response to generate effect on or to forward
     ...
     * Alternative specifications such as 'localhost' or '127.0.0.1' or an
     * actual IP of this machine are not considered command hosts. Local hosts
     * would only be command hosts if we happen to get a request from a browser
