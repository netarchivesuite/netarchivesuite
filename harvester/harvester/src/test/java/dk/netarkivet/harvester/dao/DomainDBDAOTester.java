...
public class DomainDBDAOTester extends DataModelTestCase {
	...
	public void testGetDomainJobInfo() {
		...
		long domainId = dao.queryLongValue(
				"SELECT domain_id FROM domains WHERE name=:name", 
				new ParameterMap("name", theDomainName));
		long configId = dao.queryLongValue(
				"SELECT config_id FROM configurations WHERE name=:name AND domain_id=:id",
				new ParameterMap(
						"name", configName,
						"id", domainId));
	...
	public void testGetDomainHarvestInfo() {
		...
		DataModelTestDao testDao = DataModelTestDao.getInstance();
		long domainId = testDao.queryLongValue(
				"SELECT domain_id FROM domains WHERE name=:name", 
				new ParameterMap("name", theDomainName));
		long configId = testDao.queryLongValue(
				"SELECT config_id FROM configurations WHERE name=:name AND domain_id=:id",
				new ParameterMap(
						"name", configName,
						"id", domainId));
		insertHarvestInfo(hi1, configId);
		insertHarvestInfo(hi2, configId);
		DomainDAO dao = DomainDAO.getInstance();
	...
	private void insertHarvestInfo(
		...
		try {
			...
			dao.executeUpdateGetGeneratedKeys(
					"INSERT INTO historyinfo (stopreason, objectcount, bytecount, config_id,"
							+ " job_id, harvest_id, harvest_time)"
							+ " VALUES (:stopReason, :objectCount, :byteCount, :configId,"
							+ " :jobId, :harvestId, :harvestTime)",
							new ParameterMap(
									"stopReason", harvestInfo.getCountObjectRetrieved(),
									"objectCount", harvestInfo.getCountObjectRetrieved(), 
									"byteCount", harvestInfo.getSizeDataRetrieved(),
									"configId", configId,
									"jobId", (harvestInfo.getJobID() != null ? harvestInfo.getJobID() : ""),
									"harvestId", harvestInfo.getHarvestID(),
									"harvestTime", harvestInfo.getDate()),
									keys);
			harvestInfo.setID((Long) keys.entrySet().iterator().next().getValue());
